...
An \textbf{operational amplifier} is a high-gain differential voltage amplifier with two inputs---the \textbf{inverting input} $V_-$ (marked $-$) and the \textbf{non-inverting input} $V_+$ (marked $+$)---and a single \textbf{output} $V_\text{out}$. The open-loop output voltage is $V_\text{out} = A_\text{OL}(V_+ - V_-)$, where $A_\text{OL} \sim 10^5$--$10^6$.
\end{definition}

\begin{fact}[Ideal Op-Amp Model]
An ideal op-amp has \textbf{infinite input impedance}, so no current flows into either input terminal ($I_+ = I_- = 0$), and \textbf{zero output impedance}, so the output can source or sink any current without a voltage drop. Its \textbf{open-loop gain} is infinite, $A_\text{OL} \to \infty$, and its \textbf{bandwidth} is infinite too, meaning that gain is flat at all frequencies.
\end{fact}

\begin{fact}[Golden Rules (with Negative Feedback)]
When an op-amp is connected with \emph{negative feedback} (output fed back to $V_-$), two rules follow from the ideal model. Rule 1, \textbf{no input current:} $I_+ = I_- = 0$, from the infinite input impedance. Rule 2, the \textbf{virtual short:} the op-amp drives its own output until $V_+ = V_-$, forcing the two inputs to the same voltage. Rule 2 does \emph{not} mean the inputs are connected---it means the feedback loop enforces the equality.
\end{fact}
...
\begin{circuitikz}[american]
  \draw (0,0) node[op amp] (opamp) {};
  \coordinate (aux) at (0,1.8);
  \draw (opamp.+) -- ++(-0.8,0) node[left]{$V_\text{in}$};
  \draw (opamp.out) -- ++(0.6,0) coordinate (fb) -- ++(0.5,0) node[right]{$V_\text{out}$};
  \draw (opamp.-) -- ++(-0.5,0) coordinate (m);
  \draw (m) -- (m |- aux) -- (fb |- aux) -- (fb);
  \filldraw (fb) circle (2pt);
\end{circuitikz}
\end{center}
\caption{Voltage follower: the output is wired straight back to $V_-$, so $V_\text{out} = V_\text{in}$.}
\end{figure}
...
\begin{circuitikz}[american]
  \draw (0,0) node[op amp] (opamp) {};
  \coordinate (aux) at (0,2.2);
  \draw (opamp.+) -- ++(-0.35,0) node[ground]{};
  \draw (opamp.-) -- ++(-0.9,0) coordinate (s1);
  \draw (s1) to[R, l=$R_1$] ++(-2.2,0) node[left]{$V_1$};
  \draw (s1) -- ++(0,-1.2) coordinate (s2);
  \draw (s2) to[R, l=$R_2$] ++(-2.2,0) node[left]{$V_2$};
  \draw (s2) -- ++(0,-1.2) coordinate (s3);
  \draw (s3) to[R, l=$R_3$] ++(-2.2,0) node[left]{$V_3$};
  \draw (opamp.out) -- ++(0.6,0) coordinate (fb) -- ++(0.5,0) node[right]{$V_\text{out}$};
  \draw (s1) -- (s1 |- aux) to[R, l=$R_f$] (fb |- aux) -- (fb);
  \filldraw (s1) circle (2pt);
  \filldraw (s2) circle (2pt);
  \filldraw (fb) circle (2pt);
\end{circuitikz}
\end{center}
...
\begin{circuitikz}[american]
  \draw (0,0) node[op amp] (opamp) {};
  \coordinate (aux) at (0,1.9);
  \draw (opamp.-) -- ++(-0.5,0) coordinate (m);
  \draw (m) to[R, l=$R_1$] ++(-2.0,0) node[left]{$V_1$};
  \draw (opamp.+) -- ++(-0.5,0) coordinate (p);
  \draw (p) to[R, l=$R_2$] ++(-2.0,0) node[left]{$V_2$};
  \draw (p) to[R, l_=$R_g$] ++(0,-1.7) node[ground]{};
  \draw (opamp.out) -- ++(0.6,0) coordinate (fb) -- ++(0.5,0) node[right]{$V_\text{out}$};
  \draw (m) -- (m |- aux) to[R, l=$R_f$] (fb |- aux) -- (fb);
  \filldraw (m) circle (2pt);
  \filldraw (p) circle (2pt);
  \filldraw (fb) circle (2pt);
\end{circuitikz}
\end{center}
\caption{Difference amplifier: $V_1$ into $V_-$ through $R_1$ with feedback $R_f$, $V_2$ into $V_+$ through $R_2$ with $R_g$ to ground.}
\end{figure}
...
\begin{circuitikz}[american]
  \draw (0,0) node[op amp] (opamp) {};
  \coordinate (aux) at (0,1.9);
  \draw (opamp.+) -- ++(-0.6,0) node[ground]{};
  \draw (opamp.-) -- ++(-0.9,0) coordinate (m);
  \draw (m) to[R, l=$R$] ++(-2.2,0) node[left]{$V_\text{in}$};
  \draw (opamp.out) -- ++(0.6,0) coordinate (fb) -- ++(0.5,0) node[right]{$V_\text{out}$};
  \draw (m) -- (m |- aux) to[C, l=$C$] (fb |- aux) -- (fb);
  \filldraw (m) circle (2pt);
  \filldraw (fb) circle (2pt);
\end{circuitikz}
\end{center}
...
\begin{circuitikz}[american]
  ...
  \draw (opamp.+) -- ++(-0.6,0) node[ground]{};
  \draw (opamp.-) -- ++(-0.9,0) coordinate (m);
  \draw (m) to[C, l=$C$] ++(-2.2,0) node[left]{$V_\text{in}$};
  \draw (opamp.out) -- ++(0.6,0) coordinate (fb) -- ++(0.5,0) node[right]{$V_\text{out}$};
  \draw (m) -- (m |- aux) to[R, l=$R$] (fb |- aux) -- (fb);
  \filldraw (m) circle (2pt);
  \filldraw (fb) circle (2pt);
\end{circuitikz}
\end{center}
\caption{Op-amp differentiator: capacitor at the input, resistor as the feedback element.}
\end{figure}
...
\]
where $I_s$ is the saturation (reverse leakage) current, $n \approx 1$--$2$ is the ideality factor, and $V_T = kT/q \approx 26\,\text{mV}$ at room temperature. In the forward-biased regime the drop is roughly fixed and set by the material: a silicon diode sits at $V_f \approx 0.6$--$0.7\,\text{V}$, a red, orange, or yellow LED at $V_f \approx 1.8$--$2.2\,\text{V}$, and a green, blue, or white LED at $V_f \approx 2.8$--$3.5\,\text{V}$ (the shorter the wavelength, the larger the bandgap and the higher the drop).
\end{definition}
...
A three-terminal semiconductor device with terminals \textbf{Base} (B), \textbf{Collector} (C), and \textbf{Emitter} (E). It comes in two types, distinguished by the direction of conventional current: in an \textbf{NPN} it flows \emph{into} the base and collector and out of the emitter, while in a \textbf{PNP} it flows \emph{out} of the base and collector and into the emitter. The B--E junction behaves as a diode ($V_{BE} \approx 0.7\,\text{V}$ for silicon NPN in the active region).
\end{definition}

\begin{fact}[BJT Operating Regions]
There are three regions of operation, set by how hard the base is driven. In \textbf{cutoff}, $V_{BE} < 0.6\,\text{V}$, the transistor is \emph{off} and $I_C \approx 0$. In the \textbf{active} region, $V_{BE} \approx 0.7\,\text{V}$ with $V_{CE} > 0.2\,\text{V}$, the collector current is a fixed multiple of the base current,
\[
  I_C = \beta\, I_B, \quad \beta \approx 100\text{--}300
\]
so the device acts as a linear amplifier. In \textbf{saturation}, $V_{BE} > 0.7\,\text{V}$ and $V_{CE} \approx 0.1$--$0.2\,\text{V}$, the transistor is fully \emph{on} and $I_C$ is limited by the external circuit rather than by $\beta$.
\end{fact}
...
\textbf{Design procedure:} first take the required collector current to be the load current, $I_C = I_\text{load}$. Then force saturation by choosing $I_B \geq I_C/\beta_\text{min}$; in practice use $I_B \approx I_C/10$, which overdrives the base enough that the switch stays hard on even for a low-$\beta$ part. Finally the base resistor drops whatever the B--E junction does not:
\[
...
\begin{circuitikz}[american]
  \draw (0,0) node[npn] (Q) {};
  \draw (Q.collector) -- ++(0,0.6) coordinate (cnode)
        to[R, l=$R_C$] ++(0,1.4) -- ++(0,0.2) node[above]{$V_{CC}$};
  \draw (Q.emitter) -- ++(0,-0.5) node[ground]{};
  \draw (Q.base) -- ++(-1.4,0) node[left]{$v_\text{in}$};
  \draw (cnode) -- ++(0.9,0) node[right]{$v_\text{out}$};
  \filldraw (cnode) circle (2pt);
\end{circuitikz}
\end{center}
\caption{Common-emitter stage: $R_C$ from $V_{CC}$ to the collector, emitter at ground, output at the collector (DC bias network not drawn).}
\end{figure}
...
For an RC circuit with corner (cutoff) frequency $f_c = \dfrac{1}{2\pi RC}$, the two arrangements are just the two ways of tapping the same complex voltage divider. The \textbf{low-pass filter} puts $R$ in series between input and output and $C$ from output to ground, giving
\[
  H(f) = \frac{V_\text{out}}{V_\text{in}} = \frac{1/j\omega C}{R + 1/j\omega C} = \frac{1}{1 + j\omega RC}
\]
so $|H| \approx 1$ for $f \ll f_c$ and $|H| \approx f_c/f \to 0$ for $f \gg f_c$: high frequencies are attenuated. The \textbf{high-pass filter} swaps them, $C$ in series and $R$ from output to ground, giving
\[
  H(f) = \frac{R}{R + 1/j\omega C} = \frac{j\omega RC}{1 + j\omega RC}
\]
so $|H| \approx 0$ for $f \ll f_c$ and $|H| \approx 1$ for $f \gg f_c$: low frequencies are attenuated and DC is blocked entirely. At $f = f_c$: $|H| = 1/\sqrt{2} \approx 0.707$, i.e.\ the $-3\,\text{dB}$ point; signal power is halved.
\end{fact}
...
Far from the corner frequency, the same two filters do calculus on the input. A low-pass filter driven well above its corner, $f \gg f_c$, acts as an \textbf{integrator}: the capacitor never fully charges within a cycle, so the output is essentially the running integral of the input,
\[
  V_\text{out}(t) \approx \frac{1}{RC}\int V_\text{in}\,dt.
\]
A high-pass filter driven well below its corner, $f \ll f_c$, acts as a \textbf{differentiator}: the capacitor charges quickly, so the voltage left across $R$ tracks the rate of change,
\[
  V_\text{out}(t) \approx RC\,\frac{dV_\text{in}}{dt}.
\]
\end{fact}
...
The 555 timer IC is built around a \textbf{voltage divider} of three equal resistors ($5\,\text{k}\Omega$ each, hence ``555'') which sets reference voltages at $V_{CC}/3$ and $2V_{CC}/3$. These feed two \textbf{comparators}: the upper one trips when pin 6 (threshold) rises above $2V_{CC}/3$, the lower one when pin 2 (trigger) falls below $V_{CC}/3$. Their outputs set and reset an \textbf{SR latch}, which in turn drives both the output stage and a \textbf{discharge transistor} on pin 7, and an \textbf{output buffer} sinks or sources up to $\sim 200\,\text{mA}$.
\end{definition}
...
\begin{center}
\begin{circuitikz}[american]
  \node[draw, minimum width=2.8cm, minimum height=3.6cm, font=\bfseries] (IC) at (0,0) {555};
  \node[right, font=\scriptsize] at (-1.4,1.2) {7 DIS};
  \node[right, font=\scriptsize] at (-1.4,-0.3) {6 THR};
  \node[right, font=\scriptsize] at (-1.4,-1.1) {2 TRIG};
  \node[left, font=\scriptsize] at (1.4,0.6) {OUT 3};
  \node[below right, font=\scriptsize] at (0.05,1.8) {8};
  \node[above right, font=\scriptsize] at (0.05,-1.8) {1};
  % supply rail and timing resistor
  \draw (0,1.8) -- (0,2.8) node[above]{$V_{CC}$};
  \draw (0,2.8) -- (-3.0,2.8);
  \draw (-3.0,2.8) to[R, l_=$R$] (-3.0,1.2);
  % pins 6 and 7 tied to the timing node, C to ground
  \draw (-1.4,1.2) -- (-3.0,1.2);
  \draw (-1.4,-0.3) -- (-3.0,-0.3);
  \draw (-3.0,1.2) -- (-3.0,-0.3);
  \draw (-3.0,-0.3) to[C, l_=$C$] (-3.0,-1.5) node[ground]{};
  \filldraw (-3.0,1.2) circle (2pt);
  \filldraw (-3.0,-0.3) circle (2pt);
  % trigger in, output out, ground pin
  \draw (-1.4,-1.1) -- (-2.2,-1.1) node[below, font=\small]{trigger};
  \draw (1.4,0.6) -- (2.3,0.6) node[right]{$V_\text{out}$};
  \draw (0,-1.8) -- (0,-2.3) node[ground]{};
\end{circuitikz}
\end{center}
\caption{555 monostable: $R$ charges $C$ from $V_{CC}$ with pins 6 and 7 tied together, giving a pulse of width $T = 1.1RC$ after a falling edge on pin 2.}
\end{figure}

\textbf{Operation:} the trigger pulse first sends the output HIGH and turns the discharge transistor off, which frees $C$ to charge through $R$ toward $V_{CC}$. When $V_C$ reaches $2V_{CC}/3$ the upper comparator resets the latch, the output goes LOW, and the discharge transistor turns back on and rapidly dumps the charge on $C$, leaving the circuit ready for the next trigger.
...
\begin{circuitikz}[american]
  ...
  \node[below right, font=\scriptsize] at (0.05,1.8) {8};
  \node[above right, font=\scriptsize] at (0.05,-1.8) {1};
  % supply rail, R_A to pin 7, R_B from pin 7 down to pins 6/2
  \draw (0,1.8) -- (0,2.8) node[above]{$V_{CC}$};
  \draw (0,2.8) -- (-3.0,2.8);
  \draw (-3.0,2.8) to[R, l_=$R_A$] (-3.0,1.2);
  \draw (-3.0,1.2) to[R, l_=$R_B$] (-3.0,-0.3);
  \draw (-1.4,1.2) -- (-3.0,1.2);
  \draw (-1.4,-0.3) -- (-3.0,-0.3);
  \draw (-1.4,-1.1) -- (-3.0,-1.1);
  \draw (-3.0,-0.3) -- (-3.0,-1.1);
  \draw (-3.0,-1.1) to[C, l_=$C$] (-3.0,-2.3) node[ground]{};
  \filldraw (-3.0,1.2) circle (2pt);
  \filldraw (-3.0,-0.3) circle (2pt);
  \filldraw (-3.0,-1.1) circle (2pt);
  \draw (1.4,0.6) -- (2.3,0.6) node[right]{$V_\text{out}$};
  \draw (0,-1.8) -- (0,-2.3) node[ground]{};
\end{circuitikz}
\end{center}
\caption{555 astable: $C$ charges through $R_A + R_B$ and discharges through $R_B$ into pin 7, cycling between $V_{CC}/3$ and $2V_{CC}/3$.}
\end{figure}

\textbf{Operation:} the capacitor cycles endlessly between the two comparator thresholds. It charges through $R_A + R_B$ from $V_{CC}/3$ up to $2V_{CC}/3$ with the output HIGH, then discharges through $R_B$ alone from $2V_{CC}/3$ back down to $V_{CC}/3$ with the output LOW.
