% Pages 222--244: Magnetic Field (continued), Field Transformations, Practice Test, Electromagnetic Induction intro

\chapter{Magnetic Field (Continued)}
...
In steady state the electric force exactly cancels the magnetic force:
\[
    eE = e\,\vect{v} \times \vect{B}
\]
...
The drift speed is:
\[
    v = \frac{J}{ne} = \frac{I}{neh\ell}
\]

\textbf{Hall voltage:} $V_H = |\vect{E}|\,\ell$

With $I = Jh\ell$:
\[
    V_H = EL \quad \xrightarrow{\text{defines}} \quad R_{\text{Hall}}
\]
\[
    V_H = \left(\frac{B}{neh}\right) I
\]

The Hall effect tells you the charge density $n$.
...
Let $\vect{B} = \curl \vect{A}$ (the \emph{magnetic vector potential}). Then $\div\vect{B} = 0$ is automatically satisfied because $\div(\curl \vect{A}) = 0$.
...
\chapter{Recitation 13: Magnetic Force}
...
\begin{figure}[H]
    \centering
    \caption{A Wien filter: crossed electric ($\vect{E}$, vertical) and magnetic ($\vect{B}$, into page) fields. Only particles with speed $v = E/B$ pass through undeflected.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{A square current loop of side $\ell$ and mass $m$ carrying current $I$ in a uniform magnetic field $\vect{B}$ (into the page). Gravity pulls down; the magnetic force on the bottom wire balances gravity.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Infinite current sheet in the $xz$-plane with surface current $K$ in the $z$-direction. The $dB$ from each strip is perpendicular to the position vector. Symmetry says only the $x$-component survives.}
\end{figure}
...
\chapter{Section 6.7: How the Fields Transform}
% Wednesday, April 3, 2024

\section{Field Transformation Under a Lorentz Boost}

\begin{figure}[H]
    \centering
    \caption{Two reference frames: the lab frame and a frame moving with velocity $v_0$ to the left. The charge density transforms as $\sigma' = \sigma\gamma_0 / \gamma_c$.}
\end{figure}
...
\chapter{Chapter 7: Electromagnetic Induction}
% 7.1--7.5 Textbook Summary, Saturday, April 13, 2024

\section{7.2: Conducting Rod Moving Through a Uniform Magnetic Field}

\begin{figure}[H]
    \centering
    \caption{A conducting rod of length $\ell$ moving with velocity $v$ to the right through a uniform magnetic field $\vect{B}$ pointing upward. The magnetic force on positive charges pushes them to one end, creating a charge separation.}
\end{figure}
...
\]
The EMF is the electromotive force (not actually a force --- it has units of electric potential).
\begin{itemize}
    \item \textbf{emf}: any influence that causes charge to circulate around a closed path.
    \item EMF must be non-electrostatic ($\curl\vect{E} = 0$ for electrostatic fields, which cannot drive a current around a closed loop).
\end{itemize}
\end{definition}
...
% Page 244: Section 7.6--7.10 header only (blank page, no content)
