...
\begin{proof}
Let $V$ be the cavity, bounded by the conductor's inner surface $S$. Inside the cavity $\rho = 0$, so $\phi$ satisfies Laplace's equation there, and since the conductor is an equipotential, $\phi = \phi_0$ at every point of $S$. That is a Dirichlet problem, so Theorem~\ref{thm:em-uniqueness} applies. Now simply guess a solution: the constant function $\phi \equiv \phi_0$ throughout $V$ satisfies $\laplacian\phi = 0$ and matches the boundary value everywhere on $S$. By uniqueness it is therefore \emph{the} solution, and $\vect{E} = -\grad\phi = 0$ throughout the cavity.
\end{proof}
