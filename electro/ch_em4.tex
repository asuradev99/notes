...
\label{thm:em-equipotential}
The scalar potential $\phi$ is constant throughout a conductor in electrostatic equilibrium --- both its solid interior and its entire surface, including the walls of any cavity carved into it.
\end{theorem}

\begin{proof}
Recall the line-integral relation between $\vect{E}$ and $\phi$: for any path $\gamma$ running from a point $P$ to a point $Q$,
\[
    \phi(Q) - \phi(P) = -\int_\gamma \vect{E}\cdot d\vect{\ell}.
\]

\textbf{Step 1 (the interior).} Let $P$ and $Q$ be any two interior points, and choose a path $\gamma$ from $P$ to $Q$ lying entirely within the conductor --- possible because the interior is connected (Figure~\ref{fig:em-equip-interior}). Because $\vect{E} = 0$ at \emph{every} point of the interior, the integrand vanishes identically along the whole path, whatever its shape; no appeal to path independence or to Stokes's theorem is needed. Hence
\[
    \phi(Q) - \phi(P) = -\int_\gamma 0\,d\ell = 0 .
\]
Since $P$ and $Q$ were arbitrary, $\phi \equiv \phi_0$ for a single constant $\phi_0$ throughout the interior.
...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[font=\small] at (-0.35,-0.8) {$\gamma$, with $\vect{E}=0$ along it};
\end{tikzpicture}
\end{center}
\caption{Step 1. Any two interior points are joined by a path lying wholly inside the conductor, along which the integrand vanishes identically, so all interior points share one potential $\phi_0$.}
\label{fig:em-equip-interior}
\end{figure}

\textbf{Step 2 (continuity across the surface).} Before reaching the surface we need to know that $\phi$ does not jump there. Join a point $P_\text{below}$ just inside the surface to a point $P_\text{above}$ just outside by a short path of length $h$ crossing the boundary along the normal (Figure~\ref{fig:em-equip-continuity}). Then
\[
    \bigl|\phi(P_\text{above}) - \phi(P_\text{below})\bigr|
    = \left|\int_{P_\text{below}}^{P_\text{above}} \vect{E}\cdot d\vect{\ell}\right|
    \le \Bigl(\max_{\text{path}}|\vect{E}|\Bigr)\, h .
\]
Since $\sigma$, and hence $\vect{E}$, remains finite on both sides as the surface is approached, the right-hand side vanishes as $h\to 0$. So $\phi$ is continuous across the surface, and the value at a surface point is the limit of its values along any path of interior points approaching that point.

\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=1.0]
  \fill[gray!20] (-3,-1.3) rectangle (3,0);
  \draw[thick] (-3,0) -- (3,0);
  \node[font=\small] at (0,-0.85) {conductor};
  \filldraw (0,-0.45) circle (1.3pt);
  \node[font=\small, left] at (-0.12,-0.45) {$P_\text{below}$};
  \filldraw (0,0.45) circle (1.3pt);
  \node[font=\small, right] at (0.12,0.45) {$P_\text{above}$};
  \draw[-{Stealth}, thick, Orange] (0,-0.45) -- (0,0.42);
  \draw[<->, gray, thin] (-1.5,-0.45) -- (-1.5,0.45) node[midway, left, font=\small, fill=white, inner sep=1.5pt] {$h$};
  \node[font=\small] at (2.1,-0.35) {$h\to 0$};
\end{tikzpicture}
\end{center}
\caption{Step 2. The potential difference across the surface is bounded by $|\vect{E}|_{\max}h$. Because the surface charge, and so the field, stays finite on both sides, this bound vanishes with $h$ and $\phi$ is continuous across the boundary.}
\label{fig:em-equip-continuity}
\end{figure}

This is where the finiteness of $\sigma$ does real work. The argument fails for an idealized \emph{dipole layer}, in which $\sigma\to\infty$ as the layer thickness $d\to 0$ with the product $\sigma d$ held fixed: there $|\vect{E}|h$ does not vanish in the limit, and the potential genuinely jumps across the layer. An ordinary conductor carries a finite surface charge, so the exception does not arise here.

\textbf{Step 3 (the surface).} Let $R$ be any point on the conductor's surface, and let $P_n \to R$ be a sequence of interior points approaching it, say along the inward normal (Figure~\ref{fig:em-equip-surface}). Each $P_n$ is interior, so $\phi(P_n) = \phi_0$ for every $n$ by Step 1, and by the continuity of Step 2,
\[
    \phi(R) = \lim_{n\to\infty}\phi(P_n) = \lim_{n\to\infty}\phi_0 = \phi_0 .
\]
Since $R$ was an arbitrary surface point, $\phi = \phi_0$ on the entire surface --- including the walls of any cavity hollowed out of the conductor, which are simply another part of that same surface. Combining Steps 1 and 3, $\phi = \phi_0$ at every point of the conductor.
\end{proof}
...
\begin{tikzpicture}[scale=0.9]
  \draw[fill=gray!15, thick] plot[smooth cycle, tension=0.8] coordinates {(-2.6,-1.1) (-2,1.3) (0.5,1.9) (2.6,0.6) (2.2,-1.4) (0,-1.8)};
  \filldraw (1.05,-0.55) circle (1.2pt);
  \filldraw (1.45,-0.35) circle (1.2pt);
  \filldraw (1.8,-0.15) circle (1.2pt);
  \node[font=\small, below] at (1.3,-0.62) {$P_n$};
  \filldraw (2.32,0.42) circle (1.4pt) node[right, font=\small] {$R$};
  \draw[->, dotted] (1.85,-0.11) -- (2.28,0.38);
\end{tikzpicture}
\end{center}
\caption{Step 3. Each interior point of the sequence sits at potential $\phi_0$, and continuity carries that value out to the surface point $R$ in the limit.}
\label{fig:em-equip-surface}
\end{figure}

\subsection{Shielding: No Field Lines Within a Conductor}
\label{sec:em-shielding}

An immediate corollary is that no field line can run through the material of a conductor. Such a line would join two distinct points $A$ and $B$ of the conductor with $\vect{E}\neq 0$ along it, giving $\phi(B)-\phi(A) = -\int_A^B\vect{E}\cdot d\vect{\ell} \neq 0$ and contradicting the constancy just proved. This is the basis of \textbf{electrical shielding}. It says nothing yet about a \emph{cavity} inside a conductor, where there is no material to carry the argument; that stronger statement needs the uniqueness theorem, and is proved as a corollary of it below.
...
Before proving that such a problem has only one solution, we should be precise about what counts as admissible boundary data.

\begin{definition}[Dirichlet boundary condition]
The value of the potential itself is specified at every point of the boundary surface $S = \partial V$:
\[
    \phi(\vect{x}) = f(\vect{x}) \quad\text{for } \vect{x}\in S,
\]
for some prescribed function $f$. Holding each conductor at a given potential is a Dirichlet condition.
\end{definition}

\begin{definition}[Neumann boundary condition]
The normal derivative of the potential --- equivalently the normal component of the field, since $E_n = -\pdv*{\phi}{n}$ --- is specified at every point of $S$:
\[
    \pdv{\phi}{n}(\vect{x}) = g(\vect{x}) \quad\text{for } \vect{x}\in S,
\]
where $\pdv*{}{n} \equiv \hat{n}\cdot\grad$ is the derivative along the outward normal. Specifying the surface charge on each conductor is a Neumann condition, since $\sigma = -\varepsilon_0\,\pdv*{\phi}{n}$.
\end{definition}

\begin{fact}[Neumann data cannot be prescribed arbitrarily]
Integrating Gauss's law over $V$ and applying the divergence theorem gives
\[
    \oint_S \vect{E}\cdot\hat{n}\,da = \frac{1}{\varepsilon_0}\int_V \rho\,d^3x
    \quad\Longleftrightarrow\quad
    \oint_S \pdv{\phi}{n}\,da = -\frac{1}{\varepsilon_0}\int_V \rho\,d^3x .
\]
So $g$ cannot be chosen freely: its integral over the whole boundary is fixed by the total charge enclosed. A proposed Neumann condition violating this identity corresponds to no physical configuration at all.
\end{fact}

\begin{fact}[Cauchy data is over-determined]
Specifying \emph{both} $\phi$ and $\pdv*{\phi}{n}$ on the same closed surface is over-determined. As the theorem below shows, Dirichlet data alone already fixes the solution, so imposing Neumann data on top of it is either redundant --- consistent with the Dirichlet solution --- or admits no solution at all.
\end{fact}

\begin{theorem}[Uniqueness]
\label{thm:em-uniqueness}
Let $V$ be a region with boundary $S = \partial V$, and let $\phi_1$ and $\phi_2$ both solve Poisson's equation in $V$ for the same charge density,
\[
    \laplacian\phi_1 = \laplacian\phi_2 = -\frac{\rho}{\varepsilon_0} \quad\text{in } V .
\]
If they satisfy the same boundary condition on $S$ --- either
\begin{enumerate}[label=(\alph*)]
    \item Dirichlet, $\phi_1 = \phi_2$ on $S$;
    \item Neumann, $\pdv*{\phi_1}{n} = \pdv*{\phi_2}{n}$ on $S$; or
    \item mixed, Dirichlet on one part of $S$ and Neumann on the rest
\end{enumerate}
--- then $\phi_1 = \phi_2$ throughout $V$ in cases (a) and (c), and $\phi_1 = \phi_2 + \text{const.}$ throughout $V$ in case (b).
\end{theorem}

\begin{proof}
Let $U \equiv \phi_1 - \phi_2$. Since both solve Poisson's equation for the same $\rho$, the sources cancel:
\[
    \laplacian U = \left(-\frac{\rho}{\varepsilon_0}\right) - \left(-\frac{\rho}{\varepsilon_0}\right) = 0 \quad\text{in } V,
\]
so $U$ is harmonic in $V$. Apply Green's first identity (Formula~\ref{form:em-green1}) with both functions equal to $U$:
\[
    \int_V \left(U\laplacian U + |\grad U|^2\right) d^3x = \oint_S U\,\pdv{U}{n}\,da .
\]
The first term vanishes because $U$ is harmonic, leaving
\[
    \int_V |\grad U|^2\,d^3x = \oint_S U\,\pdv{U}{n}\,da .
\]

\emph{The surface integral vanishes in every case.} In case (a), $U = 0$ on $S$, so the integrand vanishes pointwise. In case (b), $\pdv*{U}{n} = 0$ on $S$, so again it vanishes pointwise. In case (c), every point of $S$ lies in one region or the other, so at each point either $U = 0$ or $\pdv*{U}{n} = 0$, and the integrand vanishes pointwise across the whole of $S$. In all three cases
\[
    \oint_S U\,\pdv{U}{n}\,da = 0 \quad\Longrightarrow\quad \int_V |\grad U|^2\,d^3x = 0 .
\]

\emph{Conclusion.} The integrand $|\grad U|^2$ is non-negative pointwise, and a non-negative integrand integrating to zero must vanish identically. Hence $\grad U = 0$ everywhere in $V$, and $U$ is constant throughout.

In case (a) --- and in case (c), on its Dirichlet portion --- $U = 0$ at every point of that part of $S$; since $U$ is constant throughout $V$, that constant is zero, and $\phi_1 = \phi_2$ exactly. In case (b) nothing pins down the constant: only $\pdv*{U}{n} = 0$ was known on $S$, which says nothing about the value of $U$ there. So $\phi_1$ and $\phi_2$ agree up to an additive constant --- which is physically as it should be, since only $\vect{E} = -\grad\phi$ is observable and an additive constant leaves it unchanged.
\end{proof}
...
The first application is the one promised in Section~\ref{sec:em-shielding}: a conductor shields not only its own material but any cavity hollowed out of it.

\begin{theorem}[Corollary: a hollow conductor shields its interior]
In the space inside a hollow conductor of any shape, if that space is empty of charge, the electric field is zero.
\end{theorem}

\begin{proof}
Let $V$ be the cavity, bounded by the conductor's inner surface $S$. Inside the cavity $\rho = 0$, so $\phi$ satisfies Laplace's equation there, and since the conductor is an equipotential, $\phi = \phi_0$ at every point of $S$ --- Theorem~\ref{thm:em-equipotential} covers cavity walls precisely because they are part of the conductor's surface. That is a Dirichlet problem, so Theorem~\ref{thm:em-uniqueness} applies. Now simply guess a solution: the constant function $\phi \equiv \phi_0$ throughout $V$ satisfies $\laplacian\phi = 0$ and matches the boundary value everywhere on $S$. By uniqueness it is therefore \emph{the} solution, and $\vect{E} = -\grad\phi = 0$ throughout the cavity.
\end{proof}

Notice how little work this took once uniqueness was available: the hard part is knowing that a guess which fits is necessarily right. This is the same licence the method of images exploits next, and it is worth seeing it used once on a problem whose answer we can check against intuition before trusting it on one whose answer we cannot.
...
\]
Returning to the original problem, the surface boundary condition of Section~\ref{sec:em-surface} gives $E_z = \sigma/\varepsilon_0$ just above the plane, whence
\[
...
An isolated conductor carrying charge $Q$ acquires a potential $\phi_0$, measured with respect to zero at infinity. Scaling the charge by $\alpha$ scales the boundary data by $\alpha$, and $\alpha\phi$ then solves the same problem with the scaled data, so by Theorem~\ref{thm:em-uniqueness} it is the solution: the potential everywhere scales with the charge. Hence $Q\propto\phi_0$, with a constant of proportionality that depends only on the geometry.
...
For a pair of conductors the useful quantity is referred not to a potential measured from infinity but to the potential \emph{difference} between them.

\begin{definition}[Capacitor]
A \emph{capacitor} is a pair of conductors, separated by insulation, carrying equal and opposite charges $+Q$ and $-Q$, so that every field line leaving one terminates on the other. Its capacitance is
\[
    C = \frac{Q}{|\phi_1 - \phi_2|},
\]
with $Q$ the charge transferred from one conductor to the other, both having started neutral.
\end{definition}
...
This is the direct $n$-conductor generalization of $Q = C\phi$, and it makes precise the sense in which, on a system of conductors, the charges and the potentials are not independent of one another: fixing all the potentials fixes every charge, and the relation between them is linear.
