...
The sum of EMFs and potential drops around any closed loop must add up to zero. This comes from the fact that $\oint \vect{E}\cdot d\vect{s} = 0$.
\[
    \mathcal{E} = IR
\]
\end{theorem}
...
At any current junction: $I = I_1 + I_2$.
\end{theorem}
...
\begin{itemize}
    \item Power done to push an electron through an electric field $\vect{E}$ is $q\vect{E}\cdot\vect{v}$ (charge $\times$ force/second).
    \item This energy is eventually transferred to heat in the form of kinetic energy.
    \item Suppose a current $I$ travels in an electric field $\vect{E}$ in a resistor $R$ at voltage $V$. Then the net work done in $\Delta t$ is:
\end{itemize}
\[
...
\begin{itemize}
    \item EMF transports charge carriers in the \emph{opposite} direction to the direction they are supposed to be going in.
    \item Ex: Van de Graaff generator --- energy needs to be put into the system to crank the charges up the shaft; potential is $V_0$.
    \item In batteries, charge carriers move to higher energy because doing so expends less chemical energy than not.
\end{itemize}

\subsection{Example: Voltaic Cells}

\begin{itemize}
    \item Chemical source of EMF.
    \item Ex: lead-sulfuric acid cell.
\end{itemize}

\section{4.10: Networks with Voltage Sources}
...
\begin{itemize}
    \item If we don't know what is in the box, we can determine $\mathcal{E}_\text{eq}$ and $R_\text{eq}$ by:
    \begin{enumerate}
        \item Measuring the ``open circuit'' voltage between terminals A and B with a multimeter --- the voltage $= \mathcal{E}_\text{eq}$.
        \item Measuring the ``short circuit'' current by connecting them via an ammeter of negligible resistance.
    \end{enumerate}
    \[
...
\begin{itemize}
    \item $\mathcal{E}_\text{eq}$ ($= V_\text{TH}$): Calculate the open-circuit voltage between the two terminals, having removed the load connected to A and B off the circuit.
    \item $R_\text{eq}$: Connect terminals by a wire and calculate the ``short-circuit'' current $I_\text{sc}$ through this wire; then $R_\text{eq} = \mathcal{E}_\text{eq}/I_\text{sc}$ between the two ports.
    \item OR: Calculate $R_\text{eq}$ by making the internal EMFs zero (removing all the batteries) and then plug in $\mathcal{E}_\text{eq}$ and $R_\text{eq}$ into the Thevenin circuit.
\end{itemize}
