...
Before turning to any particular experiment, we extract from these assumptions a single equation of motion, which every application in the rest of the chapter is then a special case of. The object it governs is the average momentum per electron $\vect{p}(t)$, which is also what carries the current: if $n$ electrons per unit volume drift with velocity $\vect{v} = \vect{p}/m$, then in a time $dt$ all electrons within a slab of thickness $v\,dt$ cross a given face, so the current density is
\begin{equation}
    \vect{j} = -ne\vect{v} = -\frac{ne\vect{p}(t)}{m}. \label{eq:drude-j}
\end{equation}

To follow $\vect{p}$ in time, advance it from $t$ to $t+\Delta t$ and split on whether a collision occurs. With probability $1 - \Delta t/\tau$ the electron does not collide and its momentum simply advances under the external force $\vect{f}(t)$; with probability $\Delta t/\tau$ it does collide, after which --- by the equilibrium assumption, which randomizes the emerging direction --- its contribution to the average momentum vanishes, up to terms of order $\vect{f}(t)\Delta t$ picked up since the collision. Therefore
\begin{equation}
...
Collisions thus enter the dynamics as nothing more than a linear drag, with the scattering time setting its strength. What remains is to choose $\vect{f}$.

% ============================================================
\section{DC Conductivity and Ohm's Law}
% ============================================================

Take the simplest choice, a static uniform electric field, so that $\vect{f} = -e\vect{e}$. In steady state the momentum stops changing, $\dv*{\vect{p}}{t} = 0$, and~\eqref{eq:drude-eom} is solved immediately by
\begin{equation}
    \vect{p} = -e\vect{e}\tau \quad\Longleftrightarrow\quad \vect{v}_{\text{avg}} = -\frac{e\vect{e}\tau}{m}:
\end{equation}
...
The same result follows from a free-flight argument that makes the role of $\tau$ more concrete. Take an electron that emerges from a collision at $t=0$ with velocity $\vect{v}_0$ and is uniformly accelerated by $-e\vect{e}$ until its next collision, so that $\vect{v}_f = \vect{v}_0 - e\vect{e}t/m$. Averaging over electrons, the post-collision velocities point in random directions, so $\langle\vect{v}_0\rangle = 0$, and the mean time since the last collision is $\langle t\rangle = \tau$; this recovers $\vect{v}_{\text{avg}} = -e\vect{e}\tau/m$.
...
\end{equation}
the conductivity relation is a local statement about fields. To connect it to what an ammeter measures, note that the total current through a surface is $I = \vect{j}\cdot\vect{a} = jA$ when the field is normal to the surface. For a wire of length $L$ and cross section $A$ carrying uniform current (Figure~\ref{fig:drude-wire}), the voltage drop is
\begin{equation}
...
Now add a magnetic field. Place a metal slab in a field $\vect{h}$ along $\hat{z}$, drive a current $j_x$ along $\hat{x}$, and take for the external force the Lorentz force
\begin{equation}
    \vect{f} = -e\left(\vect{e} + \frac{\vect{v}}{c}\times\vect{h}\right).
\end{equation}
...
In steady state the Drude equation~\eqref{eq:drude-eom} with $\vect{v} = \vect{p}/m$ reads
\begin{equation}
...
\begin{equation}
    \mathcal{N}(\varepsilon) = \frac{2}{V}\sum_{\vect{k}} \Theta\bigl(\varepsilon - \varepsilon(\vect{k})\bigr).
\end{equation}
...
\begin{equation}
    g(\varepsilon) = \frac{2}{V}\sum_{\vect{k}} \delta\bigl(\varepsilon - \varepsilon(\vect{k})\bigr)
    = \frac{1}{4\pi^3}\int \dd\vect{k}\;\delta\bigl(\varepsilon - \varepsilon(\vect{k})\bigr),
    \label{eq:drude-dos-delta}
\end{equation}
using $\sum_{\vect{k}} \to (V/8\pi^3)\int\dd\vect{k}$. Either way, $g(\varepsilon)\,\dd\varepsilon$ is the number of one-electron levels per unit volume with energies between $\varepsilon$ and $\varepsilon + \dd\varepsilon$.
\end{definition}

The delta-function form is the more useful of the two, because it makes clear that $g$ is a property of the dispersion alone --- no reference to occupation, temperature, or the Fermi sphere --- and because it can be reduced to a surface integral. Writing $\dd\vect{k} = \dd S\,\dd k_\perp$ with $\dd S$ an element of the constant-energy surface $S(\varepsilon)$ and $\dd k_\perp$ normal to it, and using $\dd\varepsilon = |\grad_{\vect{k}}\varepsilon|\,\dd k_\perp$, the delta function integrates out in the perpendicular direction and leaves
\begin{equation}
    g(\varepsilon) = \frac{1}{4\pi^3}\int_{S(\varepsilon)} \frac{\dd S}{\left|\grad_{\vect{k}}\,\varepsilon(\vect{k})\right|}.
...
Both definitions give the same answer. Taking the counting route first, the levels with $\varepsilon(\vect{k}) = \hbar^2k^2/2m$ below $\varepsilon$ are those inside a sphere of radius $k = \sqrt{2m\varepsilon}/\hbar$, and with two spin states per $\vect{k}$,
\begin{equation}
...
\end{equation}
By the delta-function route, spherical symmetry gives $\int\dd\vect{k} = 4\pi\int k^2\dd k$, and the delta function is evaluated with $|\grad_{\vect{k}}\varepsilon| = \hbar^2 k/m$:
\begin{equation}
