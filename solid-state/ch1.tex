...
% ============================================================
\section{The Drude Equation}
% ============================================================

Before turning to any particular experiment, we extract from these assumptions a single equation of motion, which every application in the rest of the chapter is then a special case of. The object it governs is the average momentum per electron $\lvect{p}(t)$, which is also what carries the current: if $n$ electrons per unit volume drift with velocity $\lvect{v} = \lvect{p}/m$, then in a time $dt$ all electrons within a slab of thickness $v\,dt$ cross a given face, so the current density is
\begin{equation}
    \lvect{j} = -ne\lvect{v} = -\frac{ne\lvect{p}(t)}{m}. \label{eq:drude-j}
\end{equation}

To follow $\lvect{p}$ in time, advance it from $t$ to $t+\Delta t$ and split on whether a collision occurs. With probability $1 - \Delta t/\tau$ the electron does not collide and its momentum simply advances under the external force $\cvect{F}(t)$; with probability $\Delta t/\tau$ it does collide, after which --- by the equilibrium assumption, which randomizes the emerging direction --- its contribution to the average momentum vanishes, up to terms of order $\cvect{F}(t)\Delta t$ picked up since the collision. Therefore
\begin{equation}
    \lvect{p}(t+\Delta t) = \left(1 - \frac{\Delta t}{\tau}\right)\Bigl(\lvect{p}(t) + \cvect{F}(t)\Delta t + O(\Delta t^2)\Bigr) + \frac{\Delta t}{\tau}\Bigl(0 + O(\cvect{F}(t)\Delta t)\Bigr).
\end{equation}
Dropping terms of second order in $\Delta t$,
\begin{equation}
    \lvect{p}(t+\Delta t) - \lvect{p}(t) = -\frac{\Delta t}{\tau}\lvect{p}(t) + \cvect{F}(t)\Delta t + O(\Delta t)^2,
\end{equation}
and dividing by $\Delta t$ gives the central dynamical statement of the model.

\begin{formula}[Drude equation]
\begin{equation}
    \dv{\lvect{p}(t)}{t} = -\frac{\lvect{p}(t)}{\tau} + \cvect{F}(t). \label{eq:drude-eom}
\end{equation}
The first term is a damping force generated by collisions; the second is the external force.
\end{formula}

Collisions thus enter the dynamics as nothing more than a linear drag, with the scattering time setting its strength. What remains is to choose $\cvect{F}$.

% ============================================================
\section{DC Conductivity and Ohm's Law}
% ============================================================

Take the simplest choice, a static uniform electric field, so that $\cvect{F} = -e\cvect{E}$. In steady state the momentum stops changing, $\dv*{\lvect{p}}{t} = 0$, and~\eqref{eq:drude-eom} is solved immediately by
\begin{equation}
    \lvect{p} = -e\cvect{E}\tau \quad\Longleftrightarrow\quad \lvect{v}_{\text{avg}} = -\frac{e\cvect{E}\tau}{m}:
\end{equation}
the field does not accelerate the electrons indefinitely, but drives them to a terminal drift velocity at which the drag balances it. Substituting into~\eqref{eq:drude-j} gives a current proportional to the field --- which is Ohm's law, here derived rather than assumed.

\begin{formula}[Drude DC conductivity]
\begin{equation}
    \lvect{j} = \frac{ne^2\tau}{m}\cvect{E} = \sigma\cvect{E}, \qquad \sigma = \frac{ne^2\tau}{m}. \label{eq:drude-sigma}
\end{equation}
\end{formula}

The same result follows from a free-flight argument that makes the role of $\tau$ more concrete. Take an electron that emerges from a collision at $t=0$ with velocity $\lvect{v}_0$ and is uniformly accelerated by $-e\cvect{E}$ until its next collision, so that $\lvect{v}_f = \lvect{v}_0 - e\cvect{E}t/m$. Averaging over electrons, the post-collision velocities point in random directions, so $\langle\lvect{v}_0\rangle = 0$, and the mean time since the last collision is $\langle t\rangle = \tau$; this recovers $\lvect{v}_{\text{avg}} = -e\cvect{E}\tau/m$.

\subsection{Resistivity and Resistance}

Written in the equivalent forms
\begin{equation}
    \lvect{j} = \sigma\cvect{E}, \qquad \cvect{E} = \rho\,\lvect{j}, \qquad \rho = \sigma^{-1},
\end{equation}
the conductivity relation is a local statement about fields. To connect it to what an ammeter measures, note that the total current through a surface is $I = \lvect{j}\cdot\cvect{A} = jA$ when the field is normal to the surface. For a wire of length $L$ and cross section $A$ carrying uniform current (Figure~\ref{fig:drude-wire}), the voltage drop is
\begin{equation}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1.0]
        ...
        \draw[-{Stealth}, thick, Orange] (-1.1,1.55) -- (6.1,1.55) node[above left] {$\lvect{j}$};
    \end{tikzpicture}
    \caption{A uniform conductor of length $L$ and cross section $A$ carrying current density $\lvect{j}$. The voltage drop $V=EL$ and the current $I=jA$ combine into $V = IR$ with $R = \rho L/A$.}
    \label{fig:drude-wire}
\end{figure}

\subsection{Extracting the Scattering Time}

This chain of reasoning is what makes the model testable. The field $\cvect{E}$ is known and $\lvect{j}$ is obtained from the measured current, so $\sigma$ is an experimental quantity; then, given $n$, $e$ and $m$, equation~\eqref{eq:drude-sigma} determines the relaxation time $\tau$ --- a microscopic quantity read off from a macroscopic measurement. Note that $[\rho] = [\Omega]\cdot L$. Table~\ref{tab:drude-resistivity} lists resistivities of the elements at three temperatures, together with the ratio $(\rho/T)_{373\,\mathrm{K}}/(\rho/T)_{273\,\mathrm{K}}$, whose closeness to unity displays the approximately linear temperature dependence of $\rho$; these are the typical orders of magnitude of room-temperature resistivities. Table~\ref{tab:drude-tau} gives the relaxation times extracted from them via~\eqref{eq:drude-sigma}.
...
Now add a magnetic field. Place a metal slab in a field $\cvect{H}$ along $\hat{z}$, drive a current $j_x$ along $\hat{x}$, and take for the external force the Lorentz force
\begin{equation}
    \cvect{F} = -e\left(\cvect{E} + \frac{\lvect{v}}{c}\times\cvect{H}\right).
\end{equation}
The magnetic part deflects the carriers sideways, and because no current can escape through the faces of the slab, they pile up there until the transverse field they create cancels the deflection (Figure~\ref{fig:drude-hall}). The interesting question is what that transverse field turns out to be.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1.0]
        ...
        % charge separation across the two faces normal to y
        \node[Orange] at (4.4,2.62) {\small $+\,+\,+\,+\,+\,+\,+\,+\,+$};
        \draw[very thick, Dandelion, dashed] (1.35,1.88) -- (5.65,1.88);
        \foreach \x in {3.0,3.8,4.6,5.4} {\draw[-{Stealth}] (\x,2.34) -- (\x-0.45,2.04);}
        % fields and currents
        ...
        \draw[-{Stealth}, thick] (8.0,3.15) -- (7.4,2.75);
        \node at (8.35,3.3) {$E_y$};
    \end{tikzpicture}
    \caption{Hall geometry. A current $j_x$ flows along $\hat{x}$ in a magnetic field $B\hat{z}$; the Lorentz force sweeps carriers along $-\hat{y}$ until the charge piled on the two faces normal to $\hat{y}$ sets up a transverse Hall field $E_y$ that balances it and the transverse current vanishes.}
    \label{fig:drude-hall}
\end{figure}

In steady state the Drude equation~\eqref{eq:drude-eom} with $\lvect{v} = \lvect{p}/m$ reads
\begin{equation}
    \dv{\lvect{p}}{t} = -e\left(\cvect{E} + \frac{\lvect{p}}{mc}\times\cvect{H}\right) - \frac{\lvect{p}}{\tau} = 0,
\end{equation}
and with $\cvect{H} = H\hat{z}$ the cross product reduces to
\begin{equation}
...
\end{equation}
where $\omega_c$ is the cyclotron frequency. Multiplying through by $-ne\tau/m$ and using~\eqref{eq:drude-j},
\begin{equation}
    \sigma_0 E_x = \omega_c\tau\, j_y + j_x, \qquad \sigma_0 E_y = -\omega_c\tau\, j_x + j_y, \label{eq:drude-hall-pair}
\end{equation}
with $\sigma_0 = ne^2\tau/m$ the zero-field conductivity of the previous section. The pile-up is complete when no current flows across the slab, so $j_y = 0$, and the second relation gives the transverse field
\begin{equation}
    E_y = -\left(\frac{\omega_c\tau}{\sigma_0}\right) j_x = -\left(\frac{H}{nec}\right) j_x.
\end{equation}
The transverse field is therefore proportional to both the current and the applied field, and the constant of proportionality is worth naming.

\begin{formula}[Hall coefficient]
\begin{equation}
    R_H \equiv \frac{E_y}{j_x H} = -\frac{1}{nec}.
\end{equation}
\end{formula}

Two things are remarkable about this result. First, $R_H$ depends on no property of the metal except the carrier density --- not on $\tau$, not on the field strength --- so measuring it tests directly whether $n$ is what the model says it is. Second, its sign is fixed by the sign of the charge carrier: negative for electrons, and positive if some other, positively charged carrier is responsible. Setting $j_y=0$ in the first relation of~\eqref{eq:drude-hall-pair} recovers $j_x = \sigma_0 E_x$, so the longitudinal resistance
\begin{equation}
    \rho(H) = \frac{E_x}{j_x} = \sigma_0^{-1}
\end{equation}
is unchanged from its zero-field value: within this model there is no magnetoresistance at all.
...
Now drive the metal with a time-varying field $\cvect{E}(t) = \operatorname{Re}\left(\cvect{E}(\omega)e^{-i\omega t}\right)$. The Drude equation reads
\begin{equation}
    \dv{\lvect{p}}{t} = -\frac{\lvect{p}}{\tau} - e\cvect{E},
\end{equation}
and with the ansatz $\lvect{p}(t) = \operatorname{Re}\left(\lvect{p}(\omega)e^{-i\omega t}\right)$ we get
\begin{equation}
...
\begin{equation}
    \lvect{j}(\omega) = \sigma(\omega)\cvect{E}(\omega), \qquad \sigma(\omega) = \frac{\sigma_0}{1 - i\omega\tau}.
\end{equation}
\end{formula}

Two approximations underlie this result. First, the magnetic part of the electromagnetic wave has been dropped: the extra term $-e\lvect{p}/(mc)\times\cvect{H}$ is smaller than the electric term by a factor $v/c \ll 1$. Second, the field is assumed not to vary appreciably in space over the electron mean free path, i.e.\ its wavelength satisfies $\lambda \gg \ell$; otherwise the electron would sample a spatially varying field between collisions.
...
\begin{equation}
    \div \cvect{E} = 0, \quad \div\cvect{B} = 0, \quad \curl\cvect{E} = -\frac{1}{c}\pdv{\cvect{H}}{t},
\end{equation}
\begin{equation}
    \curl\cvect{H} = \frac{4\pi}{c}\lvect{j} + \frac{1}{c}\pdv{\cvect{E}}{t}
    = \frac{4\pi}{c}\sigma\cvect{E} + \frac{1}{c}\pdv{\cvect{E}}{t}
    = \frac{4\pi}{c}\sigma E - \frac{i\omega}{c}E,
\end{equation}
where in the last step the harmonic time dependence $e^{-i\omega t}$ has been used. Taking the curl of Faraday's law and using $\div\cvect{E} = 0$,
\begin{equation}
    \curl(\curl \cvect{E}) = \grad(\div\cvect{E}) - \laplacian{\cvect{E}} = -\frac{1}{c}\pdv{t}(\curl \cvect{H})
    = -\frac{1}{c}\left(-i\omega\frac{4\pi}{c}\sigma E - \frac{i\omega}{c}(-i\omega)E\right),
\end{equation}
so that
\begin{equation}
    -\laplacian{\cvect{E}} = \frac{\omega^2}{c^2}\left(1 + \frac{4\pi i \sigma}{\omega}\right)E,
\end{equation}
...
\begin{equation}
    -\frac{\hbar^2}{2m}\laplacian{\psi(\lvect{r})} = E\,\psi(\lvect{r}),
\end{equation}
whose solutions are plane waves
\begin{equation}
    \psi(\lvect{r}) = \frac{1}{\sqrt{V}}e^{i\lvect{k}\cdot\lvect{r}},
\end{equation}
with dispersion
\begin{equation}
    E(\lvect{k}) = \frac{\hbar^2|\lvect{k}|^2}{2m} = \frac{|\lvect{p}|^2}{2m},
\end{equation}
so that momentum and velocity are $\lvect{p} = \hbar\lvect{k}$ and $\lvect{v} = \hbar\lvect{k}/m$.
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1.5]
        ...
        \node[above right] at (35:0.75) {$|\lvect{k}_F|$};
    \end{tikzpicture}
    \caption{The ground state of the free electron gas: all one-electron levels with $|\lvect{k}| < k_F$ are doubly occupied, filling the Fermi sphere, and all levels outside it are empty.}
    \label{fig:drude-fermi-sphere}
\end{figure}

\begin{formula}[Fermi wavevector and Fermi energy]
\begin{equation}
    |\lvect{k}_F| = (3\pi^2 n)^{1/3}, \qquad
    E_F = \frac{\hbar^2 |\lvect{k}_F|^2}{2m} = \frac{\hbar^2(3\pi^2 n)^{2/3}}{2m},
\end{equation}
where $n = N/V$ is the electron density. The Fermi momentum $\lvect{p}_F = \hbar\lvect{k}_F$ and Fermi velocity $\lvect{v}_F = \hbar\lvect{k}_F/m$ are defined analogously.
\end{formula}
...
\begin{equation}
    E = 2\sum_{|\lvect{k}| < k_F} \frac{\hbar^2 k^2}{2m} \approx 2\cdot\frac{V}{8\pi^3}\int_{|\lvect{k}|<k_F} d\lvect{k}\ \frac{\hbar^2k^2}{2m} = \frac{1}{\pi^2}\frac{\hbar^2 k_F^5}{10m}\,V,
\end{equation}
...
In a Sommerfeld electron gas the one-electron levels are specified by $\lvect{k}$ and the spin $s$, with dispersion $\varepsilon(\lvect{k}) = \hbar^2k^2/2m$, and the levels are occupied according to the Fermi--Dirac distribution
\begin{equation}
...
\begin{equation}
    U = 2\sum_{\lvect{k}} \varepsilon(\lvect{k}) f(\lvect{k}) = \frac{2V}{8\pi^3}\int d\lvect{k}\ \varepsilon(\lvect{k})f(\lvect{k}) = V\!\int d\varepsilon\ \varepsilon\, g(\varepsilon) f(\varepsilon),
\end{equation}
\begin{equation}
    N = \frac{V}{4\pi^3}\int d\lvect{k}\ f(\lvect{k}) = V\!\int d\varepsilon\ g(\varepsilon)f(\varepsilon).
\end{equation}
Both integrals have the same structure: a sum over levels, weighted by something that depends on $\lvect{k}$ only through the energy $\varepsilon(\lvect{k})$. Whenever that is the case the $\lvect{k}$-integral can be collapsed onto a single integral over energy, and the function that does the collapsing is the density of states. It is worth defining it carefully, since it is the object through which every thermal property of the gas is computed, and since the same construction reappears for phonons and for Bloch electrons, where the dispersion is no longer a parabola.

There are two equivalent ways to say what $g(\varepsilon)$ is. The first counts levels below an energy and then differentiates; the second places a delta function on each level.

\begin{definition}[Density of states]
Let $\mathcal{N}(\varepsilon)$ be the \emph{counting function}: the number of one-electron levels per unit volume with energy less than $\varepsilon$, counting both spin states,
\begin{equation}
    \mathcal{N}(\varepsilon) = \frac{2}{V}\sum_{\lvect{k}} \Theta\bigl(\varepsilon - \varepsilon(\lvect{k})\bigr).
\end{equation}
The density of states is its derivative,
\begin{equation}
    g(\varepsilon) \equiv \dv{\mathcal{N}}{\varepsilon},
    \label{eq:drude-dos-deriv}
\end{equation}
or equivalently, differentiating the step functions directly,
\begin{equation}
    g(\varepsilon) = \frac{2}{V}\sum_{\lvect{k}} \delta\bigl(\varepsilon - \varepsilon(\lvect{k})\bigr)
    = \frac{1}{4\pi^3}\int \dd\lvect{k}\;\delta\bigl(\varepsilon - \varepsilon(\lvect{k})\bigr),
    \label{eq:drude-dos-delta}
\end{equation}
using $\sum_{\lvect{k}} \to (V/8\pi^3)\int\dd\lvect{k}$. Either way, $g(\varepsilon)\,\dd\varepsilon$ is the number of one-electron levels per unit volume with energies between $\varepsilon$ and $\varepsilon + \dd\varepsilon$.
\end{definition}

The delta-function form is the more useful of the two, because it makes clear that $g$ is a property of the dispersion alone --- no reference to occupation, temperature, or the Fermi sphere --- and because it can be reduced to a surface integral. Writing $\dd\lvect{k} = \dd S\,\dd k_\perp$ with $\dd S$ an element of the constant-energy surface $S(\varepsilon)$ and $\dd k_\perp$ normal to it, and using $\dd\varepsilon = |\grad_{\lvect{k}}\varepsilon|\,\dd k_\perp$, the delta function integrates out in the perpendicular direction and leaves
\begin{equation}
    g(\varepsilon) = \frac{1}{4\pi^3}\int_{S(\varepsilon)} \frac{\dd S}{\left|\grad_{\lvect{k}}\,\varepsilon(\lvect{k})\right|}.
    \label{eq:drude-dos-surface}
\end{equation}
The density of states is large where the dispersion is flat and small where it is steep. For free electrons the surfaces are spheres and the gradient is constant on each of them, so this reduces to something elementary --- but for a real band structure it is the general statement, and it is what will produce van Hove singularities later on.

\begin{example}[Free electron density of states]
Both definitions give the same answer. Taking the counting route first, the levels with $\varepsilon(\lvect{k}) = \hbar^2k^2/2m$ below $\varepsilon$ are those inside a sphere of radius $k = \sqrt{2m\varepsilon}/\hbar$, and with two spin states per $\lvect{k}$,
\begin{equation}
    \mathcal{N}(\varepsilon) = \frac{2}{8\pi^3}\cdot\frac{4}{3}\pi k^3 = \frac{k^3}{3\pi^2} = \frac{1}{3\pi^2}\left(\frac{2m\varepsilon}{\hbar^2}\right)^{3/2},
\end{equation}
so that
\begin{equation}
    g(\varepsilon) = \dv{\mathcal{N}}{\varepsilon} = \frac{k^2}{\pi^2}\dv{k}{\varepsilon} = \frac{m}{\pi^2\hbar^2}\sqrt{\frac{2m\varepsilon}{\hbar^2}}.
\end{equation}
By the delta-function route, spherical symmetry gives $\int\dd\lvect{k} = 4\pi\int k^2\dd k$, and the delta function is evaluated with $|\grad_{\lvect{k}}\varepsilon| = \hbar^2 k/m$:
\begin{equation}
    g(\varepsilon) = \frac{1}{4\pi^3}\int 4\pi k^2\,\dd k\;\delta\!\left(\varepsilon - \frac{\hbar^2k^2}{2m}\right)
    = \frac{k^2}{\pi^2}\cdot\frac{m}{\hbar^2 k} = \frac{mk}{\pi^2\hbar^2},
\end{equation}
which is the same function written in terms of $k$ rather than $\varepsilon$.
\end{example}

\begin{formula}[Free electron density of states]
\begin{equation}
    g(\varepsilon) = \begin{cases}\dfrac{m}{\hbar^2\pi^2}\sqrt{\dfrac{2m\varepsilon}{\hbar^2}}, & \varepsilon > 0,\\[6pt] 0, & \varepsilon < 0.\end{cases}
\end{equation}
The vanishing below $\varepsilon = 0$ is simply the statement that there are no free-electron levels of negative energy; the square root reflects the levels lying in a thin shell of the Fermi sphere, whose area grows as $k^2$.
\end{formula}
...
The number of one-electron levels in a $k$-space volume element is $(V/4\pi^3)\,d\lvect{k}$, so the number of electrons in that element is
\begin{equation}
    \frac{V}{4\pi^3}f(\varepsilon(\lvect{k}))\,d\lvect{k}.
\end{equation}
Using $\hbar\lvect{k} = m\lvect{v}$, so that $d\lvect{k} = (m/\hbar)^3 d\lvect{v}$, the number of electrons in a small volume of velocity space about $\lvect{v}$ is
\begin{equation}
    \frac{V}{4\pi^3}\left(\frac{m}{\hbar}\right)^3\frac{1}{\exp\left[\left(\tfrac{1}{2}mv^2 - \mu\right)/k_BT\right] + 1}\,d\lvect{v} \equiv f(\lvect{v})\,d\lvect{v}.
\end{equation}
The Sommerfeld theory of conduction then consists of running the Drude arguments verbatim, but replacing the Maxwell--Boltzmann velocity distribution by $f(\lvect{v})$. This is the \emph{semiclassical approximation}.
