...
The experimental window into the structure of a solid is scattering: we send radiation of a known wavelength at the sample and measure how much of it comes out in each direction. This chapter builds the apparatus needed to interpret such an experiment. We begin by stating the lattice definitions and the two scattering amplitudes --- the structure factor and the atomic form factor --- that the rest of the chapter exists to justify, and fix the conventions used to define a cross-section. We then treat the scattering of X-rays by a single free electron, first classically and then through Fermi's golden rule, and record the absorption cross-section that competes with elastic scattering. Next we set up the language of crystals --- lattices, unit cells, Bravais types --- and finally join the two threads through reciprocal space, where the Fourier transform of a periodic density turns the diffraction pattern into a lattice of its own.

% ============================================================
\section{Lattices, Structure Factors, and Form Factors}
% ============================================================

Before setting up the scattering problem it is worth recording, in bare form, the two lattice definitions and the two amplitudes that the rest of the chapter will justify and use. Both are developed properly later --- the lattice language in Section~\ref{sec:xrd-crystallography} and the reciprocal lattice in Section~\ref{sec:xrd-reciprocal} --- but they are stated here because they organise everything that follows.

\begin{definition}[Bravais lattice]
A \emph{Bravais lattice} is the set of all points with position vectors $\vect{r}$ of the form
\begin{equation}
    \vect{r} = n_1\vect{a}_1 + n_2\vect{a}_2 + n_3\vect{a}_3,
\end{equation}
where $\vect{a}_1, \vect{a}_2, \vect{a}_3$ are linearly independent and $n_1, n_2, n_3 \in \mathbb{Z}$.
\end{definition}

\begin{definition}[Reciprocal lattice]
The \emph{reciprocal lattice} is the set of wavevectors $\vect{k}$ satisfying
\begin{equation}
    e^{i\vect{k}\cdot\vect{r}} = 1 \qquad \text{for all } \vect{r} \in R,
\end{equation}
where $R$ is the Bravais lattice.
\end{definition}

A crystal is rarely a lattice of identical point scatterers: a unit cell generally contains several ions, sitting at positions $\vect{d}_j$ relative to the lattice point, and each ion scatters with its own strength. The scattering amplitude of one cell therefore splits into a sum of per-ion amplitudes carrying the phases of their positions.

\begin{formula}[Structure factor of a polyatomic crystal]
Let $\vect{k} = \vect{k}_f - \vect{k}_i$ be the scattering wavevector, i.e.\ the difference between the final and incident X-ray wavevectors. For a cell containing $n$ ions at positions $\vect{d}_j$,
\begin{equation}
    S_{\vect{k}} = \sum_{j=1}^{n} f_j(\vect{k})\,e^{i\vect{k}\cdot\vect{d}_j}. \label{eq:xrd-structure-poly}
\end{equation}
\end{formula}

\begin{definition}[Atomic form factor]
The \emph{atomic form factor} $f_j(\vect{k})$ is determined by the internal structure of the ion occupying the position $\vect{d}_j$: it is the Fourier transform of that ion's charge density $\rho_j(\vect{r})$,
\begin{equation}
    f_j(\vect{k}) = -\frac{1}{e}\int \dd\vect{r}\; e^{i\vect{k}\cdot\vect{r}}\,\rho_j(\vect{r}),
\end{equation}
normalised by the electron charge so that $f_j$ counts electrons.
\end{definition}

The remainder of the chapter earns these expressions: the form factor is what a single electron's scattering amplitude becomes once the electrons are bound into an ion, and the sum in~\eqref{eq:xrd-structure-poly} is exactly the cell integral that the Fourier transform of a periodic density will produce.

% ============================================================
\section{Cross-Section Conventions}
% ============================================================
...
\begin{equation}
    \vect{e}(\vect{r},t) = \hat{\varepsilon}\,E_0\exp\bigl(i\vect{k}\cdot\vect{r} - i\omega t\bigr),
\end{equation}
with the transversality conditions
\begin{equation}
    \hat{\varepsilon}\cdot\uvect{k} = 0, \qquad \vect{k}\cdot\vect{e} = \vect{k}\cdot\vect{h} = 0 .
\end{equation}
\end{formula}
...
\begin{equation}
    \text{number density of light} = \frac{\text{energy density}}{\text{energy per photon}} \propto \frac{c\,|\vect{e}_{\text{in}}|^2}{\hbar\omega} .
\end{equation}
