...
Write $\vect{r} \equiv (\vect{r}_1, \ldots, \vect{r}_n)$ for the electron coordinates and $\vect{R} \equiv (\vect{R}_1, \ldots, \vect{R}_n)$ for the nuclear coordinates. There are three kinds of Coulomb interaction between them:
\begin{align}
    \text{nucleus--nucleus:} \quad & \sum_{I > J} \frac{Z_I Z_J e^2}{|\vect{R}_I - \vect{R}_J|} = V_{nn}(\vect{R}), \\
    \text{electron--nucleus:} \quad & -\sum_{i, I} \frac{Z_I e^2}{|\vect{r}_i - \vect{R}_I|} = V_{ne}(\vect{r}, \vect{R}), \\
    \text{electron--electron:} \quad & \sum_{i > j} \frac{e^2}{|\vect{r}_i - \vect{r}_j|} = V_{ee}(\vect{r}),
\end{align}
so that the total potential is $V(\vect{r}, \vect{R}) = V_{nn}(\vect{R}) + V_{ne}(\vect{r}, \vect{R}) + V_{ee}(\vect{r})$ and the Schr\"odinger equation reads
\begin{equation}
    i\hbar\, \pdv{\hat{\Phi}(\vect{r}, \vect{R}, t)}{t} = \left(-\sum_I \frac{\hbar^2}{2M_I}\pdv[2]{}{\vect{R}_I} - \sum_i \frac{\hbar^2}{2m}\pdv[2]{}{\vect{r}_i} + V(\vect{r}, \vect{R})\right)\hat{\Phi}(\vect{r}, \vect{R}, t).
\end{equation}
This is hopeless as it stands. The Born--Oppenheimer approximation decouples the nuclear and electronic wavefunctions,
\begin{equation}
    \hat{\Phi}(\vect{r}, \vect{R}, t) \simeq \Phi(\vect{R})\, \Psi(\vect{r}, \vect{R})\, \exp\!\left(-i\bar{E}t/\hbar\right),
\end{equation}
leaving two time-independent problems:
\begin{align}
    \left(-\sum_i \frac{\hbar^2}{2m}\pdv[2]{}{\vect{r}_i} + V(\vect{r}, \vect{R})\right)\Psi(\vect{r} \mid \vect{R}) &= E(\vect{R})\,\Psi(\vect{r}, \vect{R}), \\
    \left(-\sum_I \frac{\hbar^2}{2M_I}\pdv[2]{}{\vect{R}_I} + E(\vect{R})\right)\Phi(\vect{R}) &= \bar{E}\,\Phi(\vect{R}).
\end{align}
The electronic eigenvalue $E(\vect{R})$ determines the \emph{potential energy surface} on which the nuclei move, and equilibrium is the condition that the force on every nucleus vanishes:
\begin{formula}[Equilibrium condition]
\begin{equation}
    \vect{F}_I = -\pdv{E(\vect{R})}{\vect{R}_I} = 0.
\end{equation}
\end{formula}
...
The prefactor is the one already familiar from mode counting: periodic boundary conditions put one allowed $\vect{k}$ in every volume $(2\pi)^3/V$ of reciprocal space, so $\sum_{\vect{k}} \to (V/8\pi^3)\int\dd\vect{k}$, and the delta function then picks out only those $\vect{k}$ whose frequency is the one asked for.

\subsection{Evaluating the Density of States}

The delta function in~\eqref{eq:phon-dos} is supported on the surface $S_s(\omega)$ of constant frequency in the Brillouin zone, so it is worth converting the volume integral into an integral over that surface. Split $\dd\vect{k}$ into an element $\dd S$ of the constant-$\omega$ surface and a perpendicular displacement $\dd k_\perp$. Moving a distance $\dd k_\perp$ off the surface changes the frequency by
\begin{equation}
    \dd\omega = \left|\grad_{\vect{k}}\,\omega(\vect{k},s)\right| \dd k_\perp,
\end{equation}
since the gradient points normal to the surface. The delta function therefore integrates to unity in the perpendicular direction at the cost of a Jacobian $|\grad_{\vect{k}}\omega|^{-1}$, leaving

\begin{formula}[Density of states as a surface integral]
\begin{equation}
    g(\omega) = \frac{V}{8\pi^3}\sum_{s=1}^{3p} \int_{S_s(\omega)} \frac{\dd S}{\left|\grad_{\vect{k}}\,\omega(\vect{k},s)\right|}.
    \label{eq:phon-dos-surface}
\end{equation}
\end{formula}

Written this way the density of states is visibly a statement about the \emph{flatness} of the dispersion: where a branch is steep, a given frequency interval is spanned by few modes and $g$ is small; where it is flat, many modes crowd into the same interval and $g$ is large. At a point where $\grad_{\vect{k}}\omega = 0$ --- a band maximum, minimum, or saddle point, which symmetry guarantees at the zone centre and at zone boundaries --- the integrand diverges and $g(\omega)$ has a \emph{van Hove singularity}. These kinks are what make a measured phonon density of states structured rather than smooth, and none of the model spectra below reproduce them.
...
\begin{equation}
    \int_0^\infty \dd\omega\, g(\omega) = \sum_{s=1}^{3p}\sum_{\vect{k}} 1 = 3pN_\ell = 3N,
...
\end{equation}
Equivalently, evaluate the definition~\eqref{eq:phon-dos} directly with $\omega = ck$. The dispersion is isotropic, so the constant-frequency surfaces are spheres of radius $\omega/c$ and $|\grad_{\vect{k}}\omega| = c$; either from~\eqref{eq:phon-dos-surface} or by doing the radial integral against the delta function,
\begin{equation}
