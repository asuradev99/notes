...
Atoms in a crystal are not static: they vibrate about their equilibrium sites, and those vibrations are collective, propagating through the lattice as waves. Quantized, these waves are the \emph{phonons}. This chapter builds the machinery of lattice dynamics --- the harmonic expansion of the lattice energy, the dynamical matrix, and the dispersion relation $\omega(\lvect{k})$ --- and then uses it to explain two thermal properties of solids: the heat capacity (Einstein and Debye models) and the thermal conductivity (which requires anharmonic phonon--phonon scattering to exist at all).

Phonons are a second ``language'' of communication with a crystal: where the electrons govern its electrical properties, the phonons govern its thermal and acoustic ones. They are correspondingly important for bulk material properties such as heat capacity, thermal conductivity, and phase stability. The theoretical thread connecting this chapter to what came before is a simple one --- chemical bonds can be approximated as harmonic oscillators, and that approximation is what determines the specific heat.

% ============================================================
\section{The Born--Oppenheimer Approximation}
% ============================================================

Write $\lvect{r} \equiv (\lvect{r}_1, \ldots, \lvect{r}_n)$ for the electron coordinates and $\cvect{R} \equiv (\cvect{R}_1, \ldots, \cvect{R}_n)$ for the nuclear coordinates. There are three kinds of Coulomb interaction between them:
\begin{align}
    \text{nucleus--nucleus:} \quad & \sum_{I > J} \frac{Z_I Z_J e^2}{|\cvect{R}_I - \cvect{R}_J|} = V_{nn}(\cvect{R}), \\
    \text{electron--nucleus:} \quad & -\sum_{i, I} \frac{Z_I e^2}{|\lvect{r}_i - \cvect{R}_I|} = V_{ne}(\lvect{r}, \cvect{R}), \\
    \text{electron--electron:} \quad & \sum_{i > j} \frac{e^2}{|\lvect{r}_i - \lvect{r}_j|} = V_{ee}(\lvect{r}),
\end{align}
so that the total potential is $V(\lvect{r}, \cvect{R}) = V_{nn}(\cvect{R}) + V_{ne}(\lvect{r}, \cvect{R}) + V_{ee}(\lvect{r})$ and the Schr\"odinger equation reads
\begin{equation}
    i\hbar\, \pdv{\hat{\Phi}(\lvect{r}, \cvect{R}, t)}{t} = \left(-\sum_I \frac{\hbar^2}{2M_I}\pdv[2]{}{\cvect{R}_I} - \sum_i \frac{\hbar^2}{2m}\pdv[2]{}{\lvect{r}_i} + V(\lvect{r}, \cvect{R})\right)\hat{\Phi}(\lvect{r}, \cvect{R}, t).
\end{equation}
This is hopeless as it stands. The Born--Oppenheimer approximation decouples the nuclear and electronic wavefunctions,
\begin{equation}
    \hat{\Phi}(\lvect{r}, \cvect{R}, t) \simeq \Phi(\cvect{R})\, \Psi(\lvect{r}, \cvect{R})\, \exp\!\left(-i\bar{E}t/\hbar\right),
\end{equation}
leaving two time-independent problems:
\begin{align}
    \left(-\sum_i \frac{\hbar^2}{2m}\pdv[2]{}{\lvect{r}_i} + V(\lvect{r}, \cvect{R})\right)\Psi(\lvect{r} \mid \cvect{R}) &= E(\cvect{R})\,\Psi(\lvect{r}, \cvect{R}), \\
    \left(-\sum_I \frac{\hbar^2}{2M_I}\pdv[2]{}{\cvect{R}_I} + E(\cvect{R})\right)\Phi(\cvect{R}) &= \bar{E}\,\Phi(\cvect{R}).
\end{align}
The electronic eigenvalue $E(\cvect{R})$ determines the \emph{potential energy surface} on which the nuclei move, and equilibrium is the condition that the force on every nucleus vanishes:
\begin{formula}[Equilibrium condition]
\begin{equation}
    \cvect{F}_I = -\pdv{E(\cvect{R})}{\cvect{R}_I} = 0.
\end{equation}
\end{formula}

\subsection{The Three Assumptions}

\begin{fact}[Assumptions of lattice dynamics]
\begin{enumerate}
    \item \emph{Born--Oppenheimer approximation:} the electrons are always instantaneously in their equilibrium configuration given the atomic configuration.
    \item \emph{Pairwise potential:} the potential can be written as a sum of pairwise terms, $W = \tfrac{1}{2}\sum_{ij}\phi_{ij}(r_{ij})$. Equilibrium occurs when the average force on each atom is zero.
    \item \emph{Harmonic approximation:} the expansion
    \begin{equation}
        W = \sum_n \sum_{s \geq 0} \frac{1}{s!} \pdv[s]{\phi}{u}\,(u_n - u_{n+1})^s
    \end{equation}
    is truncated at $0 \leq s \leq 2$. Further terms are \emph{anharmonic}.
\end{enumerate}
\end{fact}

The harmonic truncation is what makes the problem exactly solvable, and --- as we will see when we come to thermal conductivity --- it is also what makes it too good, since a purely harmonic crystal has no mechanism for phonons to scatter off each other.

% ============================================================
\section{From the Wave Equation to Three Dimensions}
% ============================================================

Within the harmonic approximation the displacements obey a wave equation, whose solutions we write as
\begin{equation}
    u(x, t) = \tilde{u}\,\exp\bigl(i[kx - \omega t]\bigr),
\end{equation}
where $u$ is the displacement from the equilibrium position, $\tilde{u}$ the amplitude, $k = 2\pi/\lambda$ the wavevector, and $\omega$ the angular frequency. The underlying equation is the one-dimensional wave equation
\begin{equation}
...
\begin{equation}
    \lvect{u}(\lvect{r}, t) = \tilde{\lvect{u}}\,\exp\!\left(i\lvect{k}\cdot\lvect{r} - i\omega t\right).
\end{equation}
...
\begin{enumerate}
    ...
    \item Each such mode has a unique frequency associated with it for a given $\lvect{k}$.
...
We now carry the three assumptions above into three dimensions. Label an atom by $j$ (its index within the unit cell) and $\ell$ (which unit cell it sits in). The total lattice energy is a sum of pair potentials,
\begin{equation}
...
\begin{equation}
    E_{\text{harm}} = \frac{1}{2}\sum_{jj',\ell\ell'} \lvect{u}^{T}(j\ell)\cdot \Phi \cdot \lvect{u}(j'\ell')
...
\begin{equation}
    \lvect{u}(j\ell) = \begin{bmatrix} u_x(j\ell) \\ u_y(j\ell) \\ u_z(j\ell)\end{bmatrix},
    \qquad
...
\begin{equation}
    E_{\text{harm}} = \frac{1}{4}\sum_{jj',\ell\ell'} \bigl[\lvect{u}(j\ell) - \lvect{u}(j'\ell')\bigr]^{T} \cdot \Phi\!\begin{pmatrix} jj' \\ \ell\ell'\end{pmatrix} \cdot \bigl[\lvect{u}(j\ell) - \lvect{u}(j'\ell')\bigr].
\end{equation}
...
\begin{formula}
\begin{equation}
    m_j\, \ddot{\lvect{u}}(j\ell, t) = -\sum_{j'\ell'} \Phi\!\begin{pmatrix} jj' \\ \ell\ell'\end{pmatrix} \lvect{u}(j'\ell', t),
\end{equation}
\end{formula}
...
\begin{equation}
    \lvect{u}(j\ell, t) = \sum_{\lvect{k},\nu} \cvect{U}(j, \lvect{k}, \nu)\,\exp\!\left(i\lvect{k}\cdot\lvect{r}(j\ell) - i\omega t\right).
\end{equation}
Substituting this into the equation of motion turns the infinite set of coupled ODEs into a finite eigenvalue problem at each $\lvect{k}$:
\begin{formula}[Lattice-dynamical eigenvalue problem]
\begin{equation}
    D(\lvect{k})\,\lvect{e}(\lvect{k}, \nu) = \omega^2(\lvect{k}, \nu)\, \lvect{e}(\lvect{k}, \nu),
    \label{eq:phon-eigen}
\end{equation}
where the polarization vector
\begin{equation}
    \lvect{e} \equiv \begin{bmatrix} \lvect{u}(1, \lvect{k}, \nu) \\ \lvect{u}(2, \lvect{k}, \nu) \\ \vdots \\ \lvect{u}(n, \lvect{k}, \nu) \end{bmatrix}
\end{equation}
has $3n$ components and describes all of the polarization information for a given mode $\lvect{k}$, for all atoms in the unit cell.
\end{formula}
Here $D(\lvect{k})$ is the $3n \times 3n$ \emph{dynamical matrix}, made up of $3\times 3$ blocks that each represent the coupling between a pair of atoms (with the masses absorbed, so that the components of $\lvect{e}$ are effectively $\sqrt{m_j}\,\lvect{u}(j,\ell)$).

\begin{fact}[Properties of the dynamical matrix]
\begin{itemize}
    \item $D(\lvect{k})$ commutes with the symmetries of the lattice.
    \item $D(\lvect{k})$ is symmetric under exchange of coordinate axes, hence Hermitian.
    \item Its eigenvalue equation~\eqref{eq:phon-eigen} produces $\omega(\lvect{k})$: the \emph{dispersion relation}. Since $D(\lvect{k})$ is $3n \times 3n$ and Hermitian, we expect $3n$ distinct dispersion bands.
\end{itemize}
\end{fact}
...
\end{equation}
Summing over all modes labelled by wavevector $\lvect{k}$ and branch $\nu$,
\begin{equation}
    E(\lvect{k}, \nu) = \sum_{\lvect{k},\nu} \hbar\omega(\lvect{k})\left(n_{\lvect{k}\nu} + \tfrac{1}{2}\right).
\end{equation}

\subsection{Average Energy of a Mode}

For a single mode of frequency $\omega(\lvect{k})$ at fixed $T, P, N$, the partition function is
\begin{align}
    Z &= \sum_n \exp\!\left(\frac{-\hbar\omega(\lvect{k})(n + \tfrac{1}{2})}{k_B T}\right) \\
      &= \exp\!\left(\frac{-\beta\hbar\omega(\lvect{k})}{2}\right)\sum_n \exp\bigl(-\beta\hbar\omega(\lvect{k})\,n\bigr)
       = \frac{\exp\!\left(-\beta\hbar\omega(\lvect{k})/2\right)}{1 - \exp\bigl(-\beta\hbar\omega(\lvect{k})\bigr)},
\end{align}
a geometric series. The average energy follows from $\langle U_{\lvect{k}}\rangle = -\partial_\beta \ln Z$:

\begin{formula}[Average energy of a mode]
\begin{equation}
    \langle U_{\lvect{k}}\rangle = \hbar\omega(\lvect{k})\left[\langle n_{\lvect{k}}\rangle + \tfrac{1}{2}\right],
    \qquad
    \langle n_{\lvect{k}}\rangle \equiv \frac{1}{\exp\!\left(\dfrac{\hbar\omega(\lvect{k})}{k_B T}\right) - 1},
\end{equation}
where $\langle n_{\lvect{k}}\rangle$ --- the Planck (Bose--Einstein) distribution --- is the average occupation of a given $\lvect{k}$-mode at temperature $T$.
\end{formula}

Each excitation mode is a boson of frequency $\omega(\lvect{k})$ and is occupied $n_B(\beta\hbar\omega(\lvect{k}))$ times on average. The physical content of the distribution is that higher-frequency (higher-energy) modes are exponentially suppressed and hence much less occupied, as Figure~\ref{fig:phon-bose} shows.
...
\begin{equation}
    \langle U \rangle = \sum_{\lvect{k},\nu} \hbar\omega(\lvect{k},\nu)\left[\langle n_{\lvect{k}}\rangle + \tfrac{1}{2}\right].
\end{equation}
...
\begin{equation}
    g(\omega) \equiv \frac{V}{8\pi^3}\sum_{s=1}^{3p} \int_{\text{BZ}} \dd\lvect{k}\;\delta\bigl(\omega - \omega(\lvect{k},s)\bigr).
    \label{eq:phon-dos}
\end{equation}
\end{definition}

The prefactor is the one already familiar from mode counting: periodic boundary conditions put one allowed $\lvect{k}$ in every volume $(2\pi)^3/V$ of reciprocal space, so $\sum_{\lvect{k}} \to (V/8\pi^3)\int\dd\lvect{k}$, and the delta function then picks out only those $\lvect{k}$ whose frequency is the one asked for.

\subsection{Evaluating the Density of States}

The delta function in~\eqref{eq:phon-dos} is supported on the surface $S_s(\omega)$ of constant frequency in the Brillouin zone, so it is worth converting the volume integral into an integral over that surface. Split $\dd\lvect{k}$ into an element $\dd S$ of the constant-$\omega$ surface and a perpendicular displacement $\dd k_\perp$. Moving a distance $\dd k_\perp$ off the surface changes the frequency by
\begin{equation}
    \dd\omega = \left|\grad_{\lvect{k}}\,\omega(\lvect{k},s)\right| \dd k_\perp,
\end{equation}
since the gradient points normal to the surface. The delta function therefore integrates to unity in the perpendicular direction at the cost of a Jacobian $|\grad_{\lvect{k}}\omega|^{-1}$, leaving

\begin{formula}[Density of states as a surface integral]
\begin{equation}
    g(\omega) = \frac{V}{8\pi^3}\sum_{s=1}^{3p} \int_{S_s(\omega)} \frac{\dd S}{\left|\grad_{\lvect{k}}\,\omega(\lvect{k},s)\right|}.
    \label{eq:phon-dos-surface}
\end{equation}
\end{formula}

Written this way the density of states is visibly a statement about the \emph{flatness} of the dispersion: where a branch is steep, a given frequency interval is spanned by few modes and $g$ is small; where it is flat, many modes crowd into the same interval and $g$ is large. At a point where $\grad_{\lvect{k}}\omega = 0$ --- a band maximum, minimum, or saddle point, which symmetry guarantees at the zone centre and at zone boundaries --- the integrand diverges and $g(\omega)$ has a \emph{van Hove singularity}. These kinks are what make a measured phonon density of states structured rather than smooth, and none of the model spectra below reproduce them.

One constraint holds whatever the dispersion: integrating~\eqref{eq:phon-dos} over all frequencies counts every mode once, so
\begin{equation}
    \int_0^\infty \dd\omega\, g(\omega) = \sum_{s=1}^{3p}\sum_{\lvect{k}} 1 = 3pN_\ell = 3N,
    \label{eq:phon-dos-sum}
\end{equation}
the total number of vibrational degrees of freedom. Any model density of states must satisfy this sum rule; it is what fixes the free parameter in each of the two models that follow.
...
The crudest guess for the dispersion is that there is none: Einstein takes every mode to have the same frequency, $\omega(\lvect{k}) = \omega_E$, so the solid is $3N$ independent oscillators, each atom sitting in its own harmonic well and vibrating in three directions. The density of states is then a single delta function, with weight fixed by the sum rule~\eqref{eq:phon-dos-sum},
\begin{equation}
    g(\omega) = 3N\,\delta(\omega - \omega_E).
\end{equation}
The frequency integral collapses onto $\omega_E$, so the internal energy is just $3N$ copies of the average energy of one mode,
\begin{equation}
    \langle E\rangle = \int_0^\infty \dd\omega\, g(\omega)\,\hbar\omega\left[n_B(\beta\hbar\omega) + \tfrac{1}{2}\right]
    = 3N\hbar\omega_E\left[\frac{1}{e^{\beta\hbar\omega_E} - 1} + \tfrac{1}{2}\right].
\end{equation}
Differentiating, and writing $x \equiv \beta\hbar\omega_E = \theta_E/T$ with $k_B\theta_E \equiv \hbar\omega_E$ the \emph{Einstein temperature},
\begin{equation}
    C_V = \pdv{\langle E\rangle}{T} = 3N\hbar\omega_E \cdot \frac{e^{x}}{(e^{x}-1)^2}\cdot\frac{\hbar\omega_E}{k_B T^2},
\end{equation}
since the zero-point half drops out.

\begin{formula}[Einstein heat capacity]
\begin{equation}
    C_V = 3Nk_B\left(\frac{\theta_E}{T}\right)^{2}\frac{e^{\theta_E/T}}{\left(e^{\theta_E/T} - 1\right)^{2}},
    \qquad \theta_E \equiv \frac{\hbar\omega_E}{k_B}.
\end{equation}
\end{formula}

The two limits are read off directly. For $T \gg \theta_E$ we have $x \ll 1$, and expanding $e^x \approx 1 + x$ gives $e^x/(e^x-1)^2 \to 1/x^2$, so
\begin{equation}
    C_V \to 3Nk_B,
\end{equation}
the classical Dulong--Petit value --- as it must, since at high temperature every one of the $3N$ oscillators carries $k_B T$ by equipartition. For $T \ll \theta_E$ the exponential dominates the denominator and
\begin{equation}
    C_V \approx 3Nk_B\left(\frac{\theta_E}{T}\right)^{2} e^{-\theta_E/T},
\end{equation}
which vanishes exponentially fast. Experiment instead finds $C_V \propto T^3$ in most insulators, so the model fails badly at low temperature.

The reason for the failure is the assumption itself. A gap $\hbar\omega_E$ between the ground state and the first excited state of every mode means that once $k_B T$ falls below it, \emph{all} modes freeze out together. But a real solid has acoustic branches with $\omega \to 0$ as $\lvect{k}\to 0$: however low the temperature, there are always long-wavelength modes cheap enough to excite, and it is those modes that carry the heat capacity at low $T$. Restoring them is exactly what Debye set out to do.
...
\begin{equation}
    \omega(k) = c\,k, \qquad k \equiv |\lvect{k}|.
\end{equation}
For each wavevector $\lvect{k}$ there are three kinds of mode --- one longitudinal and two transverse (Figure~\ref{fig:phon-polarizations}) --- and all three are assumed to travel at the same velocity.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \end{scope}
\end{tikzpicture}
\caption{The three acoustic polarizations for a given $\lvect{k}$: one longitudinal mode, in which displacements are along the propagation direction, and two mutually perpendicular transverse modes. Debye assumes all three have the same velocity.}
\label{fig:phon-polarizations}
\end{figure}

Imposing periodic (``von K\'arm\'an'') boundary conditions on a hypertorus of side $L$ gives
\begin{equation}
    \lvect{k} = \frac{2\pi}{L}(n_1, n_2, n_3),
\end{equation}
so each $\lvect{k}$-point occupies a volume $(2\pi/L)^3$ in reciprocal space, and sums over modes become integrals:
\begin{equation}
    \sum_{\lvect{k}} \approx \frac{L^3}{(2\pi)^3}\int \dd\lvect{k}.
\end{equation}
...
\end{equation}
Equivalently, evaluate the definition~\eqref{eq:phon-dos} directly with $\omega = ck$. The dispersion is isotropic, so the constant-frequency surfaces are spheres of radius $\omega/c$ and $|\grad_{\lvect{k}}\omega| = c$; either from~\eqref{eq:phon-dos-surface} or by doing the radial integral against the delta function,
\begin{equation}
    g(\omega) = \frac{V}{8\pi^3}\sum_{s=1}^{3}\int 4\pi k^2\,\dd k\;\delta(\omega - ck)
    = 3\cdot\frac{V}{8\pi^3}\cdot\frac{4\pi\omega^2}{c^3}
    = \frac{3V\omega^2}{2\pi^2 c^3},
\end{equation}
the factor of three counting the one longitudinal and two transverse polarizations.

To find the expected energy we sum over all $\lvect{k}$-modes and polarizations,
\begin{equation}
    \langle E\rangle = 3\sum_{\lvect{k}} \hbar\omega(\lvect{k})\left[n_B\bigl(\beta\hbar\omega(\lvect{k})\bigr) + \tfrac{1}{2}\right]
    = 3\,\frac{L^3}{(2\pi)^3}\int \dd\lvect{k}\; \hbar\omega(\lvect{k})\left(n_B + \tfrac{1}{2}\right),
\end{equation}
and by spherical symmetry $\int \dd\lvect{k} = 4\pi\int_0^\infty k^2\,\dd k$, so
\begin{equation}
...
\begin{equation}
    \lvect{j}_u = -\kappa \grad T, \qquad [j] = \frac{\text{energy transmitted}}{\text{unit area}\cdot\text{unit time}},
\end{equation}
...
\begin{equation}
    \lvect{j} = \frac{1}{(2\pi)^3}\int \hbar\omega\, \lvect{v}_g\, n_{\lvect{k}}\, \dd^3\lvect{k}.
\end{equation}
...
Two candidate mechanisms fail on their own. Single-phonon scattering off a static imperfection or a crystal boundary cannot change the distribution, because $\omega_{\text{in}} = \omega_{\text{out}}$. And a three-phonon process with $\lvect{k}_1 + \lvect{k}_2 = \lvect{k}_3$ conserves the total wavevector exactly; if the sum of reduced wavevectors is conserved, the average $\langle \lvect{k}\rangle$ is fixed by the Bose--Einstein distribution
\begin{equation}
    n_{\lvect{k}} = \left[\exp\!\left(\frac{\hbar\omega(\lvect{k}) - \lvect{v}\cdot\lvect{k}}{k_B T}\right) - 1\right]^{-1},
\end{equation}
...
An \emph{Umklapp} (U) process is a three-phonon process in which momentum is conserved only up to a reciprocal lattice vector $\cvect{G}$:
\begin{equation}
    \lvect{k}_1 + \lvect{k}_2 = \lvect{k}_3 + \cvect{G}.
\end{equation}
\end{definition}
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
    ...
    \begin{scope}
        ...
        \draw[-{Latex}, thick] (o) -- ++(-0.5,1.3) node[above] {$\lvect{k}$};
        \draw[-{Latex}, thick] (o) -- ++(0.7,-0.6) node[below] {$\lvect{k}'$};
        \draw[-{Latex}, thick, Periwinkle] (o) -- ++(0.9,0.6) node[right] {$\lvect{k}+\lvect{k}'$};
    \end{scope}
    % --- Umklapp scattering ---
    \begin{scope}[xshift=4.6cm]
        ...
        \draw[-{Latex}, thick] (o2) -- ++(0.7,1.1) node[above] {$\lvect{k}'$};
        \draw[-{Latex}, thick] (o2) -- ++(0.6,-0.9) node[below] {$\lvect{k}$};
        \draw[-{Latex}, thick, Periwinkle] (o2) -- ++(-1.2,0) node[left] {$\lvect{k}+\lvect{k}'+\cvect{G}$};
        \draw[-{Latex}, thick, Orange, dashed] (o2) ++(0.05,-0.22) -- ++(1.3,0) node[right] {$\cvect{G}$};
    \end{scope}
...
\begin{equation}
    \underbrace{\lvect{v}(\lvect{k},\nu)}_{\text{phonon velocity}}\cdot\underbrace{\grad T}_{\text{temp.\ gradient}}\underbrace{\left(\pdv{n(\lvect{k},\nu)}{T}\right)}_{\text{change in boson dist.}} = \underbrace{\left(\pdv{n(\lvect{k},\nu)}{t}\right)_{\text{scatter}}}_{\text{time evolution of dist.}},
\end{equation}
\end{formula}
whose solution yields the relaxation time $\tau(\lvect{k},\nu)$ entering~\eqref{eq:phon-kappa}.
...
\end{equation}
obtained from DFT or molecular dynamics calculations of the potential $V$, with space-group symmetry used to reduce the number of independent constants. The second is the determination of the thermal conductivity, by solving the Peierls--Boltzmann transport equation for $\tau(\lvect{k},\nu)$.
