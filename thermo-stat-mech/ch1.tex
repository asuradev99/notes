...
\begin{center}[DIAGRAM: left panel shows the step function $\theta(x)$ (zero for $x<0$, one for $x>0$); right panel shows the corresponding spike representing $\delta(x)$]\end{center}
...
\textbf{Example: Medical Testing.} A disease affects 1 in every million people, so $\Pr(D) = 10^{-6}$. A diagnostic test has sensitivity $\Pr(+|D) = 99\%$ and a false-positive rate of $1\%$, so $\Pr(+|\bar{D}) = 0.01$. If a randomly chosen person tests positive, what is $\Pr(D|+)$?
...
\textbf{Example: Biased Coin.} For $p_A = \tfrac{3}{4}$ and $p_B = \tfrac{1}{4}$, Shannon gives $\sigma = -\tfrac{3}{4}\log_2\tfrac{3}{4} - \tfrac{1}{4}\log_2\tfrac{1}{4} \approx 0.811$ bits per symbol. Encoding \emph{pairs} of symbols with a prefix-free binary tree (assigning $AA \to 0$, $AB \to 10$, $BA \to 110$, $BB \to 111$) achieves an average of $27/16 \approx 1.688$ bits per pair, or $\approx 0.844$ bits per symbol — close to the Shannon bound.
