...
the macroscopic response function $C_V$.
...
\begin{center}
\begin{tikzpicture}[scale=0.85]
  % Outer shell ellipse (slightly larger)
  \draw[thick, blue!60!black, fill=blue!12]
    (0,0) ellipse (2.25cm and 2.25cm);
  % Represent 3D sphere with a horizontal equator ellipse
  \draw[thick, blue!40!black]
    (0,0) ellipse (2.25cm and 0.6cm);
  % Inner sphere (slightly smaller, white fill to give shell appearance)
  ...
  % Boltzmann weight label
  \node[right, font=\small] at (2.4,-1.0) {$e^{-mv^2/2k_BT}$};
\end{tikzpicture}\\[3pt]
{\small\itshape Maxwell--Boltzmann: the shell of radius $v$ and thickness $dv$ in velocity space has volume $4\pi v^2\,dv$.}
\end{center}
...
\begin{example}
\noindent\textbf{Low-temperature limit ($T \to 0$).}
...
\medskip\noindent\textbf{High-temperature limit ($T \to \infty$).}
