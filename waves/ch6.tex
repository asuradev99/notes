...
\end{tikzpicture}
\caption{Brewster's angle. When $\theta_B + \theta_T = \pi/2$ the reflected and refracted rays are perpendicular, and the reflected $p$-polarized wave vanishes.}
\label{fig:brewster}
\end{figure}

\emph{Physical picture.} The transmitted wave drives the medium's electrons into dipole oscillation along $\vect{E}_T$, and it is this oscillating dipole that re-radiates the reflected wave. But a dipole cannot radiate along its own axis. At Brewster's angle the direction the reflected ray would have to travel is exactly along the dipole axis, so no reflected wave can be emitted --- which is why $R$ vanishes.
