...
Snell's law and the law of reflection fix the \emph{directions} of the reflected and transmitted waves, but say nothing about how the incident energy is divided between them. To find the amplitudes we must impose the electromagnetic boundary conditions at the interface, which requires being careful about two facts used silently below: that all three waves share one common frequency $\omega$, and that each one's wavenumber $k$ is fixed by that $\omega$ together with the index of refraction of the medium it occupies.

\emph{Why the frequency is shared.} The boundary conditions matching $\vect{E}$ and $\vect{B}$ across the interface must hold at every point on it for \emph{all time} $t$, not just momentarily. If the incident, reflected, and transmitted waves oscillated at different frequencies $\omega_I,\omega_R,\omega_T$, the matching condition at the interface would be an equation relating $e^{-i\omega_I t}$, $e^{-i\omega_R t}$, and $e^{-i\omega_T t}$ that must hold for every $t$ --- but sinusoids of different frequencies are linearly independent functions of $t$, so the only way such a relation can hold identically is if the frequencies coincide:
\[
\omega_I = \omega_R = \omega_T \equiv \omega.
\]
Physically, this is energy conservation one photon at a time ($E = \hbar\omega$): a static, lossless interface can redirect and redistribute the wave, but it has no mechanism to shift its frequency.

\emph{Why $k$ scales with $n$.} Within a single medium of phase velocity $v$, a wave obeys the same dispersion relation as in vacuum with $c\to v$, namely $\omega = vk$, so $k = \omega/v$. Writing $v = c/n$ in terms of the index of refraction,
\begin{formula}[Wavenumber in a medium]
\begin{equation}
    k = \frac{n\omega}{c}.
\end{equation}
\end{formula}
Since $\omega$ is now common to all three waves, $k$ is fixed purely by which medium a wave lives in: the incident and reflected waves travel in the same medium at the same speed, so $k_I = k_R = n_1\omega/c$, while the transmitted wave has the different magnitude $k_T = n_2\omega/c$. This is also the missing ingredient behind Snell's law: matching the tangential component of $\vect{k}$ across the interface, $k_I\sin\theta_I = k_T\sin\theta_T$, becomes $n_1\sin\theta_I = n_2\sin\theta_T$ the moment the common factor of $\omega/c$ cancels --- the same law derived earlier from pure path-difference interference, now recovered from the wave equation directly.

With a single shared $\omega$ established, write the three waves as
\begin{equation}
...
\end{equation}
each with its magnetic partner $\vect{B} = \hat{k}\times\vect{E}/v$. Because the phases already match along the interface (that was Snell's law, just derived again above from $k=n\omega/c$), it remains only to balance the amplitude vectors $\vect{E}_{\text{left}} = \vect{E}_{0I} + \vect{E}_{0R}$ and $\vect{E}_{\text{right}} = \vect{E}_{0T}$.
...
\begin{tikzpicture}[>=Stealth,scale=1]
  ...
  \node at (-3.1,-1.3){\small$n_2$};
  \node[above right] at (R){\small reflected};
  \node[below right] at (T){\small transmitted};
  \node[above left] at (I){\small incident};
  \draw pic["\small$\theta_I$",draw,angle radius=16,angle eccentricity=1.5] {angle=Nu--O--I};
  \draw pic["\small$\theta_R$",draw,angle radius=16,angle eccentricity=1.5] {angle=R--O--Nu};
  \draw pic["\small$\theta_T$",draw,angle radius=24,angle eccentricity=1.4] {angle=Nd--O--T};
  % E (red, in-plane, perpendicular to each ray)
  \draw[red,thick,<->] (-1.226,0.974) -- (-0.974,1.226);
  \node[red] at (-0.75,1.5) {\small$\vect{E}_I$};
  \draw[red,thick,<->] (0.974,1.226) -- (1.226,0.974);
  \node[red] at (0.75,1.5) {\small$\vect{E}_R$};
  \draw[red,thick,<->] (0.4,-1.226) -- (0.82,-1.064);
  \node[red] at (1.1,-0.75) {\small$\vect{E}_T$};
  % B (blue, out of the page)
  \draw[blue,thick] (-1.4,0.75) circle (5.5pt);
  \filldraw[blue] (-1.4,0.75) circle (1pt);
  \node[blue] at (-1.4,0.4) {\small$\vect{B}_I$};
  \draw[blue,thick] (1.4,0.75) circle (5.5pt);
  \filldraw[blue] (1.4,0.75) circle (1pt);
  \node[blue] at (1.4,0.4) {\small$\vect{B}_R$};
  \draw[blue,thick] (0.95,-1.4) circle (5.5pt);
  \filldraw[blue] (0.95,-1.4) circle (1pt);
  \node[blue] at (1.35,-1.4) {\small$\vect{B}_T$};
\end{tikzpicture}
\caption{Field geometry for $p$-polarization: $\vect{E}$ (red) lies in the plane of incidence, tilted to stay perpendicular to each ray, while $\vect{B}$ (blue, $\odot$ for out of the page) is perpendicular to that plane for all three waves.}
\label{fig:p-pol-fields}
\end{figure}
...
\begin{tikzpicture}[>=Stealth,scale=1]
  ...
  \draw pic["\small$\theta_R$",draw,angle radius=16,angle eccentricity=1.5] {angle=R--O--Nu};
  \draw pic["\small$\theta_T$",draw,angle radius=24,angle eccentricity=1.4] {angle=Nd--O--T};
  % E (red, out of the page for all three waves)
  \draw[red,thick] (-1.4,0.75) circle (5.5pt);
  \filldraw[red] (-1.4,0.75) circle (1pt);
  \node[red] at (-1.4,0.4) {\small$\vect{E}_I$};
  \draw[red,thick] (1.4,0.75) circle (5.5pt);
  \filldraw[red] (1.4,0.75) circle (1pt);
  \node[red] at (1.4,0.4) {\small$\vect{E}_R$};
  \draw[red,thick] (0.95,-1.4) circle (5.5pt);
  \filldraw[red] (0.95,-1.4) circle (1pt);
  \node[red] at (1.35,-1.4) {\small$\vect{E}_T$};
  % B (blue, in-plane, perpendicular to each ray)
  \draw[blue,thick,<->] (-1.226,0.974) -- (-0.974,1.226);
  \node[blue] at (-0.75,1.5) {\small$\vect{B}_I$};
  \draw[blue,thick,<->] (0.974,1.226) -- (1.226,0.974);
  \node[blue] at (0.75,1.5) {\small$\vect{B}_R$};
  \draw[blue,thick,<->] (0.4,-1.226) -- (0.82,-1.064);
  \node[blue] at (1.1,-0.75) {\small$\vect{B}_T$};
\end{tikzpicture}
\caption{Field geometry for $s$-polarization: $\vect{E}$ (red, $\odot$ for out of the page) points along $\hat{z}$ for all three waves, while $\vect{B}$ (blue) lies in the plane of incidence, perpendicular to each ray --- the mirror image of the $p$-polarization geometry in Figure~\ref{fig:p-pol-fields}.}
\label{fig:s-pol-fields}
\end{figure}
