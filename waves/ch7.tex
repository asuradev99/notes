...
With the general machinery in place, it is worth working through a few concrete cases. Each one exercises the same two ingredients --- the interference factor set by the spacing $d$ and the envelope set by the width $a$ --- but in a slightly different regime, and together they illustrate how principal maxima, minima, and missing orders are read off directly from the formulas.

\subsection{Four Narrow Slits}
...
\subsection{Widening the Slit Spacing: $d \to 3d$}
...
\subsection{Double-Slit with Finite Width}
...
\subsection{Complementary Apertures}
