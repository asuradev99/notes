...
Every feature of this pattern comes directly from the zeros of its numerator and denominator: numerator and denominator vanishing together gives a principal maximum (L'H\^opital's rule then gives its height $N^2I_0$), while a lone numerator zero gives a minimum.

\begin{fact}[Maxima and minima of the $N$-slit pattern]
\begin{align}
    \delta_{\max} &= 2\pi n \quad (d\sin\theta = n\lambda), & \langle I \rangle_{\max} &= N^2 I_0, \\
    \delta_{\min} &= \frac{2\pi n}{N}. &&
\end{align}
Between consecutive principal maxima: $N-1$ minima and $N-2$ secondary maxima.
\end{fact}

These secondary maxima, sandwiched between minima, can never coincide with one, and carry very little of the total power. For $N=2$ the pattern collapses to the familiar two-slit case, with no minima or secondary maxima between principal maxima at all.

\subsection{Sanity Checks and Structure}

As a check, setting $N=2$ and using $\sin(\delta) = 2\sin(\delta/2)\cos(\delta/2)$ recovers the two-slit pattern:
\begin{equation}
    \langle I \rangle = I_0\left[\frac{2\sin(\delta/2)\cos(\delta/2)}{\sin(\delta/2)}\right]^2 = 4 I_0 \cos^2(\delta/2),
\end{equation}
as expected.

Figure~\ref{fig:nslit-intensity} plots the general pattern for $N=6$. The tall principal maxima are pinned to $\delta = 2\pi n$ and reach $N^2 I_0$, while the low ripples between them are the $N-2$ secondary maxima --- so faint that on the scale of the principal peaks they are barely visible.
...
\begin{fact}[Maxima and minima of the single-slit pattern]
\begin{align}
    \beta_{\max} &= 0 \quad (\theta=0), & \langle I\rangle_{\max} &= I_0, \\
    \beta_{\min} &= m\pi \quad (m\ne 0) \quad (\sin\theta = m\lambda/a), \\
    \beta_{\text{sec}} &\approx \left(m+\tfrac{1}{2}\right)\pi.
\end{align}
\end{fact}

The zeros of $\sin\beta$ give the minima directly; the central lobe spans $2\lambda/a$ between the first two, twice the angular width of every other lobe. The secondary maxima sit only approximately midway between consecutive minima (setting the derivative to zero gives $\tan\beta=\beta$, whose roots are close to but not exactly at half-integer multiples of $\pi$), and their heights fall off rapidly as $1/\beta^2$. This behavior is plotted in Figure~\ref{fig:sinc}.
...
The physical interpretation follows the two length scales in the aperture. Because $a \ll d$ in most experiments, the $\sin^2\beta/\beta^2$ envelope varies slowly with $\theta$ and encodes information about the slit \emph{width}, while the $\cos^2$ factor oscillates rapidly and encodes the slit \emph{spacing}. The peaks of the fast interference pattern are modulated by the slow single-slit envelope, as Figure~\ref{fig:combined} shows.

\begin{fact}[Maxima and minima of the finite-width double-slit pattern]
\begin{align}
    \sin\theta_{\max} &= \frac{n\lambda}{d}, \\
    \sin\theta_{\min} &= \frac{(n+\tfrac12)\lambda}{d} \quad \text{or} \quad \frac{m\lambda}{a}.
\end{align}
\end{fact}

The interference factor sets the candidate maxima, but each is scaled by the envelope $\sin^2\beta/\beta^2$ evaluated at that same angle; whenever a candidate maximum coincides with an envelope minimum, it is extinguished entirely --- a \emph{missing order}. The pattern's true minima sit wherever either factor vanishes, hence the two cases above.
...
\end{equation}
again a product of a single-slit envelope and a multi-slit interference factor. The maxima and minima follow the same rule as above with the $N$-slit factor in place of the two-slit one: principal maxima at $\sin\theta = n\lambda/d$ (unless nulled by an envelope zero, again a missing order), $N-1$ minima between each pair of them from the interference factor, and additional minima at every envelope zero $\sin\theta = m\lambda/a$.
...
\end{equation}
\end{formula}
so different wavelengths appear at different angles: the grating disperses light into a spectrum, as sketched in Figure~\ref{fig:grating-dispersion}. The minima and secondary maxima between orders are exactly the $N-1$ minima and $N-2$ secondary maxima of the $N$-slit pattern derived earlier, just with $N$ now in the thousands: the secondary maxima are utterly negligible, and the principal maxima are correspondingly razor-sharp. Because the width of each principal maximum scales as $1/N$, a grating with many rulings produces very sharp spectral lines and thus high wavelength resolution --- the reason gratings replaced prisms in serious spectroscopy.
...
\begin{equation}
    R \equiv \frac{\lambda}{\Delta\lambda} = nN.
\end{equation}
\end{fact}

Two wavelengths $\lambda$ and $\lambda+\Delta\lambda$ are \emph{just resolved} at order $n$ when the principal maximum of one coincides with the first minimum of the other; more lines or a higher order both tighten the resolving power $R$.

\begin{example}[Deriving the resolving power $R = nN$]
The order-$n$ principal maximum of wavelength $\lambda$ sits at $d\sin\theta = n\lambda$. Its first minima sit a half-width away at $d\sin\theta = n\lambda \pm \lambda/N$, the $\delta = 2\pi/N$ half-width derived above translated into $d\sin\theta$ via $\delta = 2\pi d\sin\theta/\lambda$. The same order for a second wavelength $\lambda+\Delta\lambda$ sits at $d\sin\theta = n(\lambda+\Delta\lambda)$. Setting this exactly on the first minimum of the first pattern,
\[
    n(\lambda + \Delta\lambda) = n\lambda + \frac{\lambda}{N}
    \quad\Longrightarrow\quad
    n\,\Delta\lambda = \frac{\lambda}{N},
\]
which is exactly $R = \lambda/\Delta\lambda = nN$.
\end{example}
