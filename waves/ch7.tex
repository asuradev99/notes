...
Every feature of this pattern comes directly from the zeros of its numerator and denominator. \emph{Principal maxima} sit at $\delta_{\max} = 2\pi n$ (equivalently $d\sin\theta = n\lambda$) --- independent of $N$ --- where numerator and denominator vanish together and L'H\^opital's rule gives $\langle I\rangle \to N^2 I_0$. Between two consecutive principal maxima the numerator alone produces $N-1$ \emph{minima}, at $\delta_{\min} = 2\pi n/N$; squeezed between those minima are $N-2$ much smaller \emph{secondary maxima} (which, being sandwiched between minima, can never coincide with one) that carry very little of the total power. For $N=2$ this collapses to the familiar two-slit case, with no minima or secondary maxima between principal maxima at all.
\end{fact}
...
There is a single \emph{central maximum} at $\beta = 0$ ($\theta = 0$), where $\langle I\rangle = I_0$. \emph{Minima} (zeros) sit at $\beta = m\pi$ for nonzero integer $m$, i.e.\ $\sin\theta = m\lambda/a$; the central lobe therefore spans $2\lambda/a$ between the first two minima, twice the angular width of every other lobe. Between consecutive minima sit much weaker \emph{secondary maxima}, located only approximately midway at $\beta \approx (m+\tfrac{1}{2})\pi$, whose heights fall off rapidly as $1/\beta^2$.
\end{fact}

This behavior is plotted in Figure~\ref{fig:sinc}.
...
Candidate \emph{maxima} come from the fast interference factor, at $\sin\theta = n\lambda/d$, but each is scaled by the slow envelope $\sin^2\beta/\beta^2$ evaluated at that same angle. Whenever a candidate maximum coincides with an envelope \emph{minimum} $\sin\theta = m\lambda/a$, it is extinguished entirely --- a \emph{missing order}. The pattern's true \emph{minima} sit wherever either factor vanishes: at $\sin\theta = (n+\tfrac12)\lambda/d$ from the interference factor, or $\sin\theta = m\lambda/a$ from the envelope.
\end{fact}
