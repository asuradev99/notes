...
% ============================================================
\section{Domain of Validity and Terminology}
% ============================================================
...
A lens is two spherical surfaces in series. Applying the paraxial single-surface equation to each in turn, with the image formed by the first serving as the object for the second, and then letting the thickness $t \to 0$, gives the two central results for a lens of index $n$ in air:
...
% ============================================================
\section{Sign Conventions and Magnification}
% ============================================================

Every formula above is signed, and the convention is one rule: \textbf{positive means on the side where the light actually is}.
...
% ============================================================
\section{Ray Tracing and Compound Systems}
% ============================================================

Locating an image needs no algebra at all. Three rays leaving the object tip have known behaviour, drawn in Figure~\ref{fig:ray-trace}:
\begin{enumerate}
    \item a ray parallel to the axis refracts through the far focal point;
    \item a ray through the near focal point emerges parallel to the axis;
    \item a ray through the lens centre passes undeviated.
\end{enumerate}
Any two locate the image; the third is a check. For mirrors the analogues reflect rather than refract.
...
\begin{tikzpicture}[>=Stealth, scale=0.95]
  ...
  \def\so{5}
  \def\ho{1.5}
  \pgfmathsetmacro{\si}{1/(1/\f-1/\so)}
  \pgfmathsetmacro{\hi}{-\ho*\si/\so}
  % axis and lens
  \draw[dashed] (-6.2,0) -- (5.2,0);
  \draw[thick,fill=blue!10] (0,0) ellipse (0.22 and 2.25);
  % foci
  \filldraw (-\f,0) circle (1.6pt) node[below] {\small$F$};
  \filldraw (\f,0) circle (1.6pt) node[below right] {\small$F$};
  % object and image: both stand on the optical axis (base at y=0)
  \draw[very thick,Green,->] (-\so,0) -- (-\so,\ho);
  \filldraw[Green] (-\so,0) circle (1.8pt);
  \node[Green] at (-\so,{\ho+0.35}) {\small object};
  \draw[very thick,RedOrange,->] (\si,0) -- (\si,\hi);
  \filldraw[RedOrange] (\si,0) circle (1.8pt);
  \node[RedOrange] at ({\si+1.1},{\hi+0.55}) {\small image};
  % ray 1: parallel then through far focus
  \draw[blue!70!black,thick,->] (-\so,\ho) -- (0,\ho);
  \draw[blue!70!black,thick] (0,\ho) -- (\si,\hi);
  \draw[blue!70!black,thick,->] (\si,\hi) -- ($(0,\ho)!1.22!(\si,\hi)$);
  % ray 2: through near focus then parallel
  \draw[red!75!black,thick,->] (-\so,\ho) -- (0,\hi);
  \draw[red!75!black,thick,->] (0,\hi) -- (4.6,\hi);
  % ray 3: through centre
  \draw[green!45!black,thick,->] (-\so,\ho) -- (\si,\hi);
  \draw[green!45!black,thick,->] (\si,\hi) -- ($(-\so,\ho)!1.18!(\si,\hi)$);
  % distance markers
  \draw[<->] (-\so,-2.85) -- (0,-2.85) node[midway,fill=white,inner sep=1pt] {\small$s_o$};
  \draw[<->] (0,-2.85) -- (\si,-2.85) node[midway,fill=white,inner sep=1pt] {\small$s_i$};
\end{tikzpicture}
\caption{Ray tracing through a thin converging lens. The parallel ray (blue) refracts through the far focus, the focal ray (red) emerges parallel, and the central ray (green) passes undeviated. All three meet at the image tip, which here is real and inverted, as $s_o > f$ requires.}
\label{fig:ray-trace}
\end{figure}

When the object sits \emph{inside} the focal length the same three rays no longer converge on the outgoing side: they diverge, and only their backward extensions meet. That intersection is a virtual image --- upright, magnified, on the same side as the object --- which is exactly how a magnifying glass works, and is drawn in Figure~\ref{fig:virtual-image}.
...
\begin{tikzpicture}[>=Stealth, scale=0.95]
  ...
  \def\so{1.8}
  \def\ho{1.0}
  \pgfmathsetmacro{\si}{1/(1/\f-1/\so)}   % negative: virtual
  \pgfmathsetmacro{\hi}{-\ho*\si/\so}
  \draw[dashed] (-6.6,0) -- (4.6,0);
  \draw[thick,fill=blue!10] (0,0) ellipse (0.22 and 2.5);
  \filldraw (-\f,0) circle (1.6pt) node[below] {\small$F$};
  \filldraw (\f,0) circle (1.6pt) node[below] {\small$F$};
  % object and virtual image: both stand on the optical axis (base at y=0)
  \draw[very thick,Green,->] (-\so,0) -- (-\so,\ho);
  \filldraw[Green] (-\so,0) circle (1.8pt);
  \node[Green,left] at ({-\so-0.12},{\ho*0.55}) {\small object};
  \draw[very thick,RedOrange,->] (\si,0) -- (\si,\hi);
  \filldraw[RedOrange] (\si,0) circle (1.8pt);
  \node[RedOrange] at (\si,{\hi+0.35}) {\small virtual image};
  % parallel ray -> through far focus
  \draw[blue!70!black,thick,->] (-\so,\ho) -- (0,\ho);
  \draw[blue!70!black,thick,->] (0,\ho) -- (4.4,{\ho-\ho*4.4/\f});
  \draw[blue!70!black,thick,dotted] (0,\ho) -- (\si,\hi);
  % central ray
  \draw[green!45!black,thick,->] (-\so,\ho) -- (3.6,{-\ho*3.6/\so});
  \draw[green!45!black,thick,dotted] (0,0) -- (\si,\hi);
  \node[align=center,gray] at (-4.3,-1.6)
    {\small dotted: backward extensions,\\ \small meeting at the virtual image};
\end{tikzpicture}
\caption{Object inside the focal length ($s_o < f$). The refracted rays diverge, so no real image forms; their backward extensions (dotted) meet at an upright, magnified virtual image on the object's own side, with $s_i < 0$ and $m > 1$ --- the magnifying glass.}
\label{fig:virtual-image}
\end{figure}
...
% ============================================================
\section{Dispersion and Chromatic Effects}
% ============================================================

Section~\ref{sec:dispersion} derived why $n$ depends on frequency at all, tracing it to driven bound electrons and the Sellmeier form $n^2(\omega)$. In the visible, between resonances, $n$ rises gently with $\omega$: blue light refracts more strongly than red. Everything in this chapter that involves refraction therefore acquires a colour dependence.

Three consequences matter. A prism separates white light into a spectrum with violet deviated most. The primary rainbow, formed by one internal reflection inside a spherical droplet, sits at about $42^\circ$ from the antisolar point with red on the outside. And any lens has a focal length that varies with colour --- \textbf{chromatic aberration} --- since the lensmaker's equation carries $n$ explicitly and $f \propto 1/(n-1)$.

Mirrors escape this entirely: the law of reflection contains no index at all, so $f = R/2$ holds for every wavelength equally. That immunity, together with the freedom to make a paraboloid stigmatic on axis, is why every large telescope is built around a mirror rather than a lens.
